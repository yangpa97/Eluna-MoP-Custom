...
static void CheckFields(Eluna *E, ElunaQuery *result) {
  ...
  if (field >= count) {
    char arr[256];
    sprintf(arr,
            "trying to access invalid field index %u. There are %u fields "
            "available and the indexes start from 0",
            field, count);
...
/**
 * Returns `true` if the specified column of the current row is `NULL`,
 * otherwise `false`.
 *
 * @param uint32 column
 * @return bool isNull
...
/**
 * Returns the data in the specified column of the current row, casted to a
 * boolean.
 *
 * @param uint32 column
 * @return bool data
...
/**
 * Returns the data in the specified column of the current row, casted to an
...
/**
 * Returns a table from the current row where keys are field names and values
 * are the row's values.
 *
 * All numerical values will be numbers and everything else is returned as a
 * string.
 *
 ...
 *
 * @return table rowData : table filled with row columns and data where
 * `T[column] = data`
 */
