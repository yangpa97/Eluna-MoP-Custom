...
int GetRealmID(Eluna *E) {
  E->Push(eConfigMgr->GetIntDefault("RealmID", 1));
  return 1;
}
 ...
 *
 * - For TrinityCore returns the date of the last revision, e.g. `2015-08-26
 * 22:53:12 +0300`
...
/**
 * Returns the [Map] pointer of the Lua state. Returns null for the "World"
 * state.
 *
 * @return [Map] map
 */
...
/**
 * Returns the instance ID of the Lua state. Returns 0 for continent maps and
 * the world state.
 *
 ...
 *
 * @param ObjectGuid guid : guid of the [Player], you can get it with
 * [Object:GetGUID]
...
int GetGameTime(Eluna *E) {
  E->Push(uint32(sWorld->GetGameTime()));
  return 1;
}
 ...
 * In multistate, this method is only available in the WORLD state
 *
 * @param [TeamId] team = TEAM_NEUTRAL : optional check team of the [Player],
 * Alliance, Horde or Neutral (All)
...
int GetPlayersInWorld(Eluna *E) {
  ...
  SF_SHARED_GUARD lock(*HashMapHolder<Player>::GetLock());
  ...
  for (HashMapHolder<Player>::MapType::const_iterator it = m.begin();
    ...
    if (Player *player = it->second) {
      ...
      if ((team == TEAM_NEUTRAL || uint32(player->GetTeamId()) == team) &&
          (!onlyGM || player->IsGameMaster())) {
        E->Push(player);
 ...
 * In multistate, this method is only available in the WORLD state
 *
 * @param uint32 mapId : see
 * [Map.dbc](https://github.com/cmangos/issues/wiki/Map.dbc)
 * @param uint32 instanceId = 0 : required if the map is an instance, otherwise
 * don't pass anything
...
int GetPlayerGUID(Eluna *E) {
  ...
  E->Push(ObjectGuid(MAKE_NEW_GUID(lowguid, 0, HIGHGUID_PLAYER)));
  return 1;
}
...
int GetItemGUID(Eluna *E) {
  uint32 lowguid = E->CHECKVAL<uint32>(1);
  E->Push(ObjectGuid(MAKE_NEW_GUID(lowguid, 0, HIGHGUID_ITEM)));
  return 1;
}
 ...
 *
 * A [Player] and GameObject for example can have the same low GUID but not
 * GUID.
 *
...
int GetObjectGUID(Eluna *E) {
  ...
  E->Push(ObjectGuid(MAKE_NEW_GUID(lowguid, entry, HIGHGUID_GAMEOBJECT)));
  return 1;
}
...
int GetUnitGUID(Eluna *E) {
  uint32 lowguid = E->CHECKVAL<uint32>(1);
  uint32 entry = E->CHECKVAL<uint32>(2);
  E->Push(ObjectGuid(MAKE_NEW_GUID(lowguid, entry, HIGHGUID_UNIT)));
  return 1;
}
 ...
 *
 * A GUID consists of a low GUID, type ID, and possibly an entry ID depending on
 * the type ID.
 *
 ...
 * For example creatures in instances are assigned new GUIDs when the Map is
 * created.
 *
 * On MaNGOS and cMaNGOS low GUIDs are unique only on the same map.
 * For example creatures in instances use the same low GUID assigned for that
 * spawn in the database. This is why to identify a creature you have to know
 * the instanceId and low GUID. See [Map:GetIntstanceId]
 *
 ...
 * @param [LocaleConstant] locale = DEFAULT_LOCALE : locale to return the [Item]
 * name in
...
int GetItemLink(Eluna *E) {
  ...
  if (ItemLocale const *il = eObjectMgr->GetItemLocale(entry))
    ObjectMgr::GetLocaleString(il->Name, static_cast<LocaleConstant>(locale),
                               name);

  std::ostringstream oss;
  oss << "|c" << std::hex << ItemQualityColors[temp->Quality] << std::dec
      << "|Hitem:" << entry << ":0:" << "0:0:0:0:" << "0:0:0:0|h[" << name
      << "]|h|r";
...
int GetGUIDType(Eluna *E) {
  ObjectGuid guid = E->CHECKVAL<ObjectGuid>(1);
  E->Push(static_cast<int>(GUID_HIPART(guid)));
  return 1;
}
 ...
 * @return uint32 entry : entry ID, or `0` if `guid` is not a [Creature] or
 * [GameObject]
 */
int GetGUIDEntry(Eluna *E) {
  ObjectGuid guid = E->CHECKVAL<ObjectGuid>(1);
  E->Push(GUID_ENPART(guid));
  return 1;
}
 ...
 * @param [LocaleConstant] locale = DEFAULT_LOCALE : locale to return the name
 * in
...
int GetActiveGameEvents(Eluna *E) {
  ...
  int tbl = lua_gettop(E->L);
  uint32 counter = 1;
  GameEventMgr::ActiveEvents const &activeEvents =
      eGameEventMgr->GetActiveEventList();

  for (GameEventMgr::ActiveEvents::const_iterator i = activeEvents.begin();
 ...
 * @values [5, SERVER_EVENT_ON_PACKET_RECEIVE, "WORLD", <event: number, packet:
 * WorldPacket, player: Player>, "Player only if accessible. Can return false,
 * newPacket"]
 * @values [6, SERVER_EVENT_ON_PACKET_RECEIVE_UNKNOWN, "", "", "Not
 * Implemented"]
 * @values [7, SERVER_EVENT_ON_PACKET_SEND, "WORLD", <event: number, packet:
 * WorldPacket, player: Player>, "Player only if accessible. Can return false"]
 * @values [8, WORLD_EVENT_ON_OPEN_STATE_CHANGE, "WORLD", <event: number, open:
 * boolean>, "Needs core support on Mangos"]
 * @values [9, WORLD_EVENT_ON_CONFIG_LOAD, "WORLD", <event: number, reload:
 * boolean>, ""]
 * @values [11, WORLD_EVENT_ON_SHUTDOWN_INIT, "WORLD", <event: number, code:
 * number, mask: number>, ""]
 ...
 * @values [16, ELUNA_EVENT_ON_LUA_STATE_CLOSE, "ALL", <event: number>,
 * "Triggers just before shutting down the Eluna state (on shutdown, restart and
 * reload)"]
 ...
 * @values [21, MAP_EVENT_ON_PLAYER_ENTER, "MAP", <event: number, map: Map,
 * player: Player>, ""]
 * @values [22, MAP_EVENT_ON_PLAYER_LEAVE, "MAP", <event: number, map: Map,
 * player: Player>, ""]
 * @values [23, MAP_EVENT_ON_UPDATE, "MAP", <event: number, map: Map, diff:
 * number>, ""]
 * @values [24, TRIGGER_EVENT_ON_TRIGGER, "MAP", <event: number, player: Player,
 * triggerId: number>, "Can return true"]
 * @values [25, WEATHER_EVENT_ON_CHANGE, "WORLD", <event: number, zoneId:
 * number, state: number, grade: number>, ""]
 * @values [26, AUCTION_EVENT_ON_ADD, "WORLD", <event: number, auctionId:
 * number, owner: Player, item: Item, expireTime: number, buyout: number,
 * startBid: number, currentBid: number, bidderGUIDLow: number>, ""]
 * @values [27, AUCTION_EVENT_ON_REMOVE, "WORLD", <event: number, auctionId:
 * number, owner: Player, item: Item, expireTime: number, buyout: number,
 * startBid: number, currentBid: number, bidderGUIDLow: number>, ""]
 * @values [28, AUCTION_EVENT_ON_SUCCESSFUL, "WORLD", <event: number, auctionId:
 * number, owner: Player, item: Item, expireTime: number, buyout: number,
 * startBid: number, currentBid: number, bidderGUIDLow: number>, ""]
 * @values [29, AUCTION_EVENT_ON_EXPIRE, "WORLD", <event: number, auctionId:
 * number, owner: Player, item: Item, expireTime: number, buyout: number,
 * startBid: number, currentBid: number, bidderGUIDLow: number>, ""]
 * @values [30, ADDON_EVENT_ON_MESSAGE, "WORLD", <event: number, sender: Player,
 * type: number, prefix: string, msg: string, target:
 * nil|Player|Guild|Group|number>, "Target can be
 * nil/whisper_target/guild/group/channel. Can return false"]
 * @values [31, WORLD_EVENT_ON_DELETE_CREATURE, "MAP", <event: number, creature:
 * Creature>, ""]
 * @values [32, WORLD_EVENT_ON_DELETE_GAMEOBJECT, "MAP", <event: number,
 * gameobject: GameObject>, ""]
 * @values [33, ELUNA_EVENT_ON_LUA_STATE_OPEN, "ALL", <event: number>, "Triggers
 ...
 * @values [5, ON_SPELL_CAST, "MAP", <event: number, player: Player, spell:
 * Spell, skipCheck: boolean>, ""]
 * @values [6, ON_KILL_PLAYER, "MAP", <event: number, killer: Player, killed:
 * Player>, ""]
 * @values [7, ON_KILL_CREATURE, "MAP", <event: number, killer: Player, killed:
 * Creature>, ""]
 * @values [8, ON_KILLED_BY_CREATURE, "MAP", <event: number, killer: Creature,
 * killed: Player>, ""]
 * @values [9, ON_DUEL_REQUEST, "MAP", <event: number, target: Player,
 * challenger: Player>, ""]
 * @values [10, ON_DUEL_START, "MAP", <event: number, player1: Player, player2:
 * Player>, ""]
 * @values [11, ON_DUEL_END, "MAP", <event: number, winner: Player, loser:
 * Player, type: number>, ""]
 * @values [12, ON_GIVE_XP, "MAP", <event: number, player: Player, amount:
 * number, victim: Unit>, "Can return new XP amount"]
 * @values [13, ON_LEVEL_CHANGE, "MAP", <event: number, player: Player,
 * oldLevel: number>, ""]
 * @values [14, ON_MONEY_CHANGE, "MAP", <event: number, player: Player, amount:
 * number>, "Can return new money amount"]
 * @values [15, ON_REPUTATION_CHANGE, "MAP", <event: number, player: Player,
 * factionId: number, standing: number, incremental: boolean>, "Can return new
 * standing"]
 * @values [16, ON_TALENTS_CHANGE, "MAP", <event: number, player: Player,
 * points: number>, ""]
 * @values [17, ON_TALENTS_RESET, "MAP", <event: number, player: Player, noCost:
 * boolean>, ""]
 * @values [18, ON_CHAT, "WORLD", <event: number, player: Player, msg: string,
 * Type: number, lang: number>, "Can return false, newMessage"]
 * @values [19, ON_WHISPER, "WORLD", <event: number, player: Player, msg:
 * string, Type: number, lang: number, receiver: Player>, "Can return false,
 * newMessage"]
 * @values [20, ON_GROUP_CHAT, "WORLD", <event: number, player: Player, msg:
 * string, Type: number, lang: number, group: Group>, "Can return false,
 * newMessage"]
 * @values [21, ON_GUILD_CHAT, "WORLD", <event: number, player: Player, msg:
 * string, Type: number, lang: number, guild: Guild>, "Can return false,
 * newMessage"]
 * @values [22, ON_CHANNEL_CHAT, "WORLD", <event: number, player: Player, msg:
 * string, Type: number, lang: number, channel: number>, "Can return false,
 * newMessage"]
 * @values [23, ON_EMOTE, "MAP", <event: number, player: Player, emote: number>,
 * "Not triggered on any known emote"]
 * @values [24, ON_TEXT_EMOTE, "MAP", <event: number, player: Player, textEmote:
 * number, emoteNum: number, guid: number>, ""]
 * @values [25, ON_SAVE, "MAP", <event: number, player: Player>, ""]
 * @values [26, ON_BIND_TO_INSTANCE, "MAP", <event: number, player: Player,
 * difficulty: number, mapid: number, permanent: boolean>, ""]
 * @values [27, ON_UPDATE_ZONE, "MAP", <event: number, player: Player, newZone:
 * number, newArea: number>, ""]
 * @values [28, ON_MAP_CHANGE, "MAP", <event: number, player: Player>, ""]
 * @values [29, ON_EQUIP, "MAP", <event: number, player: Player, item: Item,
 * bag: number, slot: number>, ""]
 * @values [30, ON_FIRST_LOGIN, "WORLD", <event: number, player: Player>, ""]
 * @values [31, ON_CAN_USE_ITEM, "MAP", <event: number, player: Player,
 * itemEntry: number>, "Can return InventoryResult enum value"]
 * @values [32, ON_LOOT_ITEM, "MAP", <event: number, player: Player, item: Item,
 * count: number>, ""]
 * @values [33, ON_ENTER_COMBAT, "MAP", <event: number, player: Player, enemy:
 * Unit>, ""]
 ...
 * @values [37, ON_LOOT_MONEY, "MAP", <event: number, player: Player, amount:
 * number>, ""]
 * @values [38, ON_QUEST_ABANDON, "MAP", <event: number, player: Player,
 * questId: number>, ""]
 * @values [39, ON_LEARN_TALENTS, "MAP", <event: number, player: Player,
 * talentId: number, talentRank: number, spellid: number>, ""]
 * @values [40, ON_ENVIRONMENTAL_DEATH, "MAP", <event: number, player: Player,
 * environmentalDamageType: number>, ""]
 * @values [41, ON_TRADE_ACCEPT, "MAP", <event: number, player: Player, target:
 * Player>, "Can return false to interrupt trade"]
 * @values [42, ON_COMMAND, "WORLD", <event: number, player: Player, command:
 * string>, "Player is nil if command used from console. Can return false"]
 * @values [43, ON_SKILL_CHANGE, "MAP", <event: number, player: Player, skillId:
 * number, skillValue: number>, "Returns new skill level value"]
 * @values [44, ON_LEARN_SPELL, "MAP", <event: number, player: Player, spellId:
 * number>, ""]
 * @values [45, ON_ACHIEVEMENT_COMPLETE, "MAP", <event: number, player: Player,
 * achievementId: number>, ""]
 * @values [46, ON_DISCOVER_AREA, "MAP", <event: number, player: Player, area:
 * number>, ""]
 * @values [47, ON_UPDATE_AREA, "MAP", <event: number, player: Player, oldArea:
 * number, newArea: number>, ""]
 * @values [48, ON_TRADE_INIT, "MAP", <event: number, player: Player, target:
 * Player>, "Can return false to interrupt trade"]
 * @values [49, ON_SEND_MAIL, "MAP", <event: number, player: Player,
 * recipientGuid: number>, "Can return false to interrupt sending"]
 * @values [54, ON_QUEST_STATUS_CHANGED, "MAP", <event: number, player: Player,
 ...
 * @table
 * @columns [ID, Event, State, Parameters, Comment]
 * @values [1, ON_ADD_MEMBER, "WORLD", <event: number, guild: Guild, player:
 * Player, rank: number>, ""]
 * @values [2, ON_REMOVE_MEMBER, "WORLD", <event: number, guild: Guild, player:
 * Player, isDisbanding: boolean>, ""]
 * @values [3, ON_MOTD_CHANGE, "WORLD", <event: number, guild: Guild, newMotd:
 * string>, ""]
 * @values [4, ON_INFO_CHANGE, "WORLD", <event: number, guild: Guild, newInfo:
 * string>, ""]
 * @values [5, ON_CREATE, "WORLD", <event: number, guild: Guild, leader: Player,
 * name: string>, "Not on TC"]
 * @values [6, ON_DISBAND, "WORLD", <event: number, guild: Guild>, ""]
 * @values [7, ON_MONEY_WITHDRAW, "WORLD", <event: number, guild: Guild, player:
 * Player, amount: number, isRepair: boolean>, "Can return new money amount"]
 * @values [8, ON_MONEY_DEPOSIT, "WORLD", <event: number, guild: Guild, player:
 * Player, amount: number>, "Can return new money amount"]
 * @values [9, ON_ITEM_MOVE, "WORLD", <event: number, guild: Guild, player:
 * Player, item: Item, isSrcBank: boolean, srcContainer: number, srcSlotId:
 * number, isDestBank: boolean, destContainer: number, destSlotId: number>,
 * "TODO"]
 * @values [10, ON_EVENT, "WORLD", <event: number, guild: Guild, eventType:
 * number, plrGUIDLow1: number, plrGUIDLow2: number, newRank: number>, "TODO"]
 * @values [11, ON_BANK_EVENT, "WORLD", <event: number, guild: Guild, eventType:
 * number, tabId: number, playerGUIDLow: number, itemOrMoney: number,
 * itemStackCount: number, destTabId: number>, ""]
 *
 * @proto cancel = (event, function)
 * @proto cancel = (event, function, shots)
 *
 ...
 * @table
 * @columns [ID, Event, State, Parameters, Comment]
 * @values [1, ON_MEMBER_ADD, "WORLD", <event: number, group: Group, guid:
 * number>, ""]
 * @values [2, ON_MEMBER_INVITE, "WORLD", <event: number, group: Group, guid:
 * number>, ""]
 * @values [3, ON_MEMBER_REMOVE, "WORLD", <event: number, group: Group, guid:
 * number, method: number, kicker: Player, reason: string>, ""]
 * @values [4, ON_LEADER_CHANGE, "WORLD", <event: number, group: Group,
 * newLeaderGuid: number, oldLeaderGuid: number>, ""]
 * @values [5, ON_DISBAND, "WORLD", <event: number, group: Group>, ""]
 * @values [6, ON_CREATE, "WORLD", <event: number, group: Group, leaderGuid:
 * number, groupType: number>, ""]
 * @values [7, ON_MEMBER_ACCEPT, "WORLD", <event: number, group: Group, player:
 * Player>, "Can return false to disable accepting"]
 *
 * @proto cancel = (event, function)
 * @proto cancel = (event, function, shots)
 *
 ...
 * @table
 * @columns [ID, Event, State, Parameters, Comment]
 * @values [1, ON_START, "MAP", <event: number, bg: BattleGround, bgId: number,
 * instanceId: number>, Needs to be added to TC"]
 * @values [2, ON_END, "MAP", <event: number, bg: BattleGround, bgId: number,
 * instanceId: number, winner: number>, Needs to be added to TC"]
 * @values [3, ON_CREATE, "MAP", <event: number, bg: BattleGround, bgId: number,
 * instanceId: number>, Needs to be added to TC"]
 * @values [4, ON_PRE_DESTROY, "MAP", <event: number, bg: BattleGround, bgId:
 * number, instanceId: number>, Needs to be added to TC"]
 *
 * @proto cancel = (event, function)
 * @proto cancel = (event, function, shots)
 *
 ...
 * @table
 * @columns [ID, Event, State, Parameters, Comment]
 * @values [5, ON_PACKET_RECEIVE, "WORLD", <event: number, packet: WorldPacket,
 * player: Player>, "Player only if accessible. Can return false, newPacket"]
 * @values [6, ON_PACKET_RECEIVE_UNKNOWN, "", "", "Not implemented"]
 * @values [7, ON_PACKET_SEND, "WORLD", <event: number, packet: WorldPacket,
 * player: Player>, "Player only if accessible. Can return false"]
 *
 ...
 * @table
 * @columns [ID, Event, State, Parameters, Comment]
 * @values [1, ON_HELLO, "MAP", <event: number, player: Player, object:
 * Creature|GameObject|Item>, "Object is the Creature/GameObject/Item. Can
 * return false to do default action."]
 * @values [2, ON_SELECT, "MAP", <event: number, player: Player, object:
 * Creature|GameObject|Item|Player, sender: number, intid: number, code: string,
 * menu_id: number>, "Object is the Creature/GameObject/Item/Player, menu_id is
 * only for player gossip. Can return false to do default action."]
 *
 * @proto cancel = (entry, event, function)
 * @proto cancel = (entry, event, function, shots)
 *
 ...
 * @table
 * @columns [ID, Event, State, Parameters, Comment]
 * @values [1, ON_DUMMY_EFFECT, "MAP", <event: number, caster: WorldObject,
 * spellid: number, effindex: number, item: Item>, ""]
 * @values [2, ON_USE         , "MAP", <event: number, player: Player, item:
 * Item, target: nil|GameObject|Item|Corpse|Unit|WorldObject>, "Can return false
 * to stop the spell casting"]
 * @values [3, ON_QUEST_ACCEPT, "MAP", <event: number, player: Player, item:
 * Item, quest: Quest>, "Can return true"]
 * @values [4, ON_EXPIRE      , "MAP", <event: number, player: Player, itemid:
 * number>, "Can return true"]
 * @values [5, ON_REMOVE      , "MAP", <event: number, player: Player, item:
 * Item>, "Can return true"]
 * @values [6, ON_ADD         , "MAP", <event: number, player: Player, item:
 * Item>, ""]
 * @values [8, ON_UNEQUIP     , "MAP", <event: number, player: Player, item:
 * Item, slot: number>, ""]
 * @values [7, ON_EQUIP       , "MAP", <event: number, player: Player, item:
 * Item, slot: number>, ""]
 *
 * @proto cancel = (entry, event, function)
 * @proto cancel = (entry, event, function, shots)
 ...
 * @values [1, ON_HELLO, "MAP", <event: number, player: Player, object:
 * Creature|GameObject|Item>, "Object is the Creature/GameObject/Item. Can
 * return false to do default action. For item gossip can return false to stop
 * spell casting."]
 * @values [2, ON_SELECT, "MAP", <event: number, player: Player, object:
 * Creature|GameObject|Item|Player, sender: number, intid: number, code: string,
 * menu_id: number>, "Object is the Creature/GameObject/Item/Player, menu_id is
 ...
 * @table
 * @columns [ID, Event, State, Parameters, Comment]
 * @values [1, ON_INITIALIZE                 , "MAP", <event: number,
 * instance_data: string, map: Map>, ""]
 * @values [2, ON_LOAD                       , "MAP", <event: number,
 * instance_data: string, map: Map>, ""]
 * @values [3, ON_UPDATE                     , "MAP", <event: number,
 * instance_data: string, map: Map, diff: number>, ""]
 * @values [4, ON_PLAYER_ENTER               , "MAP", <event: number,
 * instance_data: string, map: Map, player: Player>, ""]
 * @values [5, ON_CREATURE_CREATE            , "MAP", <event: number,
 * instance_data: string, map: Map, creature: Creature>, ""]
 * @values [6, ON_GAMEOBJECT_CREATE          , "MAP", <event: number,
 * instance_data: string, map: Map, go: GameObject>, ""]
 * @values [7, ON_CHECK_ENCOUNTER_IN_PROGRESS, "MAP", <event: number,
 * instance_data: string, map: Map>, ""]
 *
 ...
 * In Compatibility mode, all events are registered to the WORLD state (-1).
 *
 * Note that you can not use `ON_HELLO` with this hook. It does nothing since
 * players dont have an "on hello".
 *
 * @table
 * @columns [ID, Event, State, Parameters, Comment]
 * @values [1, ON_HELLO, "MAP", <event: number, player: Player, object:
 ...
 * @table
 * @columns [ID, Event, State, Parameters, Comment]
 * @values [1,  ON_ENTER_COMBAT, "MAP", <event: number, creature: Creature,
 * target: Unit>, "Can return true to stop normal action"]
 * @values [2,  ON_LEAVE_COMBAT, "MAP", <event: number, creature: Creature>,
 * "Can return true to stop normal action"]
 * @values [3,  ON_TARGET_DIED, "MAP", <event: number, creature: Creature,
 * victim: Unit>, "Can return true to stop normal action"]
 * @values [4,  ON_DIED, "MAP", <event: number, creature: Creature, killer:
 * Unit>, "Can return true to stop normal action"]
 * @values [5,  ON_SPAWN, "MAP", <event: number, creature: Creature>, "Can
 * return true to stop normal action"]
 * @values [6,  ON_REACH_WP, "MAP", <event: number, creature: Creature, type:
 * number, id: number>, "Can return true to stop normal action"]
 * @values [7,  ON_AIUPDATE, "MAP", <event: number, creature: Creature, diff:
 * number>, "Can return true to stop normal action"]
 * @values [8,  ON_RECEIVE_EMOTE, "MAP", <event: number, creature: Creature,
 * player: Player, emoteid: number>, "Can return true to stop normal action"]
 * @values [9,  ON_DAMAGE_TAKEN, "MAP", <event: number, creature: Creature,
 * attacker: Unit, damage: number>, "Can return true to stop normal action, can
 * return new damage as second return value."]
 * @values [10, ON_PRE_COMBAT, "MAP", <event: number, creature: Creature,
 * target: Unit>, "Can return true to stop normal action"]
 * @values [12, ON_OWNER_ATTACKED, "MAP", <event: number, creature: Creature,
 * target: Unit>, "Can return true to stop normal action. Not on mangos"]
 * @values [13, ON_OWNER_ATTACKED_AT, "MAP", <event: number, creature: Creature,
 * attacker: Unit>, "Can return true to stop normal action. Not on mangos"]
 * @values [14, ON_HIT_BY_SPELL, "MAP", <event: number, creature: Creature,
 * caster: Unit, spellid: number>, "Can return true to stop normal action"]
 * @values [15, ON_SPELL_HIT_TARGET, "MAP", <event: number, creature: Creature,
 * target: Unit, spellid: number>, "Can return true to stop normal action"]
 * @values [19, ON_JUST_SUMMONED_CREATURE, "MAP", <event: number, creature:
 * Creature, summon: Creature>, "Can return true to stop normal action"]
 * @values [20, ON_SUMMONED_CREATURE_DESPAWN, "MAP", <event: number, creature:
 * Creature, summon: Creature>, "Can return true to stop normal action"]
 * @values [21, ON_SUMMONED_CREATURE_DIED, "MAP", <event: number, creature:
 * Creature, summon: Creature, killer: Unit>, "Can return true to stop normal
 * action. Not on mangos"]
 * @values [22, ON_SUMMONED, "MAP", <event: number, creature: Creature,
 * summoner: Unit>, "Can return true to stop normal action"]
 * @values [23, ON_RESET, "MAP", <event: number, creature: Creature>, ""]
 * @values [24, ON_REACH_HOME, "MAP", <event: number, creature: Creature>, "Can
 * return true to stop normal action"]
 * @values [26, ON_CORPSE_REMOVED, "MAP", <event: number, creature: Creature,
 * respawndelay: number>, "Can return true to stop normal action, can return new
 * respawndelay as second return value"]
 * @values [27, ON_MOVE_IN_LOS, "MAP", <event: number, creature: Creature, who:
 * Unit>, "Can return true to stop normal action. Does not actually check LOS,
 * just uses the sight range"]
 * @values [30, ON_DUMMY_EFFECT, "MAP", <event: number, caster: WorldObject,
 * spellid: number, effindex: number, creature: Creature>, ""]
 * @values [31, ON_QUEST_ACCEPT, "MAP", <event: number, player: Player,
 * creature: Creature, quest: Quest>, "Can return true"]
 * @values [34, ON_QUEST_REWARD, "MAP", <event: number, player: Player,
 * creature: Creature, quest: Quest, opt: number>, "Can return true"]
 * @values [35, ON_DIALOG_STATUS, "MAP", <event: number, player: Player,
 * creature: Creature>, ""]
 ...
 * @table
 * @columns [ID, Event, State, Parameters, Comment]
 * @values [1,  ON_AIUPDATE, "MAP", <event: number, go: GameObject, diff:
 * number>, ""]
 * @values [2,  ON_SPAWN, "MAP", <event: number, go: GameObject>, ""]
 * @values [3,  ON_DUMMY_EFFECT, "MAP", <event: number, caster: WorldObject,
 * spellid: number, effindex: number, go: GameObject>, "Can return true to stop
 * normal action"]
 * @values [4,  ON_QUEST_ACCEPT, "MAP", <event: number, player: Player, go:
 * GameObject, quest: Quest>, "Can return true to stop normal action"]
 * @values [5,  ON_QUEST_REWARD, "MAP", <event: number, player: Player, go:
 * GameObject, quest: Quest, opt: number>, "Can return true to stop normal
 * action"]
 * @values [6,  ON_DIALOG_STATUS, "MAP", <event: number, player: Player, go:
 * GameObject>, ""]
 * @values [7,  ON_DESTROYED, "MAP", <event: number, go: GameObject, attacker
 * Unit>, ""]
 * @values [8,  ON_DAMAGED, "MAP", <event: number, go: GameObject, attacker:
 * Unit>, ""]
 * @values [9,  ON_LOOT_STATE_CHANGE, "MAP", <event: number, go: GameObject,
 * state: number>, ""]
 * @values [10, ON_GO_STATE_CHANGED, "MAP", <event: number, go: GameObject,
 * state: number>, ""]
 * @values [12, ON_ADD, "MAP", <event: number, go: GameObject>, ""]
 * @values [13, ON_REMOVE, "MAP", <event: number, go: GameObject>, ""]
 * @values [14, ON_USE, "MAP", <event: number, go: GameObject, player: Player>,
 * "Can return true to stop normal action"]
 *
 * @proto cancel = (entry, event, function)
 * @proto cancel = (entry, event, function, shots)
 ...
 * @table
 * @columns [ID, Event, State, Parameters, Comment]
 * @values [1, ON_CAST, "MAP", <event: number, spell: Spell, skipCheck:
 * boolean>, ""]
 *
 * @proto cancel = (entry, event, function)
 * @proto cancel = (entry, event, function, shots)
...
int RunCommand(Eluna *E) {
  ...
  eWorld->QueueCliCommand(new CliCommandHolder(
      nullptr, command, [](void *, const char *) {}, [](void *, bool) {}));
  return 0;
}
 ...
 *   (i.e. execution halts until the query has finished and then results are
 * returned).
 *
 *     local Q = WorldDBQuery("SELECT entry, name FROM creature_template LIMIT
 * 10") if Q then repeat local entry, name = Q:GetUInt32(0), Q:GetString(1)
 *             print(entry, name)
 *         until not Q:NextRow()
 *     end
 *
 * @warning This method is flagged as **unsafe** and is **disabled by default**.
 * Use with caution, or transition to Async queries.
 *
 ...
 * If you need to execute the query synchronously, use [Global:WorldDBQuery]
 * instead.
 *
...
/**
 * Initiates an asynchronous SQL query on the world database with a callback
 * function.
 *
 * The query is executed asynchronously, and the provided Lua function is called
 * when the query completes. The callback function parameter is the query result
 * (an [ElunaQuery] or nil if no rows found).
 *
 *     WorldDBQueryAsync("SELECT entry, name FROM creature_template LIMIT 10",
 * function(results) if results then repeat local entry, name =
 * results:GetUInt32(0), results:GetString(1) print(entry, name) until not
 * results:NextRow() end end)
 *
 * @param string sql : query to execute asynchronously
 * @param function callback : the callback function to be called with the query
 * results
 */
int WorldDBQueryAsync(Eluna *E) { return WorldDBQuery(E); }
 ...
 *
 * The query may be executed *asynchronously* (at a later, unpredictable time).
 * If you need to execute the query synchronously, use [Global:CharDBQuery]
 * instead.
 *
 * Any results produced are ignored.
...
/**
 * Initiates an asynchronous SQL query on the character database with a callback
 * function.
 *
 * The query is executed asynchronously, and the provided Lua function is called
 * when the query completes. The callback function parameter is the query result
 ...
 * results
 */
int CharDBQueryAsync(Eluna *E) { return CharDBQuery(E); }
 ...
 *
 * The query may be executed *asynchronously* (at a later, unpredictable time).
 * If you need to execute the query synchronously, use [Global:AuthDBQuery]
 * instead.
 *
 * Any results produced are ignored.
...
/**
 * Initiates an asynchronous SQL query on the login database with a callback
 * function.
 *
 * The query is executed asynchronously, and the provided Lua function is called
 * when the query completes. The callback function parameter is the query result
 ...
 * results
 */
int AuthDBQueryAsync(Eluna *E) { return AuthDBQuery(E); }

/**
 * Registers a global timed event.
 *
 * When the passed function is called, the parameters `(eventId, delay,
 * repeats)` are passed to it.
 *
 ...
 * @param table delaytable : a table `{min, max}` containing the minimum and
 * maximum delay time
 * @param uint32 repeats = 1 : how many times for the event to repeat, 0 is
 * infinite
...
int CreateLuaEvent(Eluna *E) {
  ...
  if (functionRef != LUA_REFNIL && functionRef != LUA_NOREF) {
    E->eventMgr->GetGlobalProcessor(GLOBAL_EVENTS)
        ->AddEvent(functionRef, min, max, repeats);
    E->Push(functionRef);
  }
  return 1;
}
...
int RemoveEventById(Eluna *E) {
  ...
  if (all_Events)
    ...
    E->eventMgr->GetGlobalProcessor(GLOBAL_EVENTS)
        ->SetState(eventId, LUAEVENT_STATE_ABORT);
  return 0;
}
...
int RemoveEvents(Eluna *E) {
  ...
  if (all_Events)
    ...
    E->eventMgr->GetGlobalProcessor(GLOBAL_EVENTS)
        ->SetStates(LUAEVENT_STATE_ABORT);
  return 0;
}
 ...
 *
 * @param int32 spawnType : type of object to spawn, 1 = [Creature], 2 =
 * [GameObject]
 ...
 * @param uint32 instanceId : instance ID to put the [Creature] or [GameObject]
 * in. Non instance is 0
 ...
 * @param bool save = false : optional to save the [Creature] or [GameObject] to
 * the database
 * @param uint32 durorresptime = 0 : despawn time of the [Creature] if it's not
 * saved or respawn time of [GameObject]
...
int PerformIngameSpawn(Eluna *E) {
  ...
  if (spawntype == 1) // spawn creature
  {
    if (save) {
      ...
      if (!creature->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_UNIT), map,
                            entry, 0, 0, x, y, z, o)) {
        delete creature;
        E->Push();
        return 1;
      ...
      // current "creature" variable is deleted and created fresh new, otherwise
      // old values might trigger asserts or cause undefined behavior
      ...
      eObjectMgr->AddCreatureToGrid(db_guid,
                                    eObjectMgr->GetCreatureData(db_guid));
      E->Push(creature);
    } else {
      TempSummon *creature =
          map->SummonCreature(entry, pos, NULL, durorresptime);
      if (!creature) {
        E->Push();
        return 1;
      }
      ...
      if (durorresptime)
        creature->SetTempSummonType(
            TempSummonType::TEMPSUMMON_TIMED_OR_DEAD_DESPAWN);
      else
        creature->SetTempSummonType(TempSummonType::TEMPSUMMON_MANUAL_DESPAWN);
  ...
  if (spawntype == 2) // Spawn object
  {
    const GameObjectTemplate *objectInfo =
        eObjectMgr->GetGameObjectTemplate(entry);
    if (!objectInfo) {
      E->Push();
      return 1;
    }
    ...
    if (!object->Create(guidLow, objectInfo->entry, map, x, y, z, o, 0, 0, 0, 0,
                        100, GOState::GO_STATE_READY, 0)) {
      delete object;
      E->Push();
      return 1;
    }
 ...
 * @param uint32 incrtime : combined with maxcount, incrtime tells how often (in
 * seconds) the vendor list is refreshed and the limited [Item] copies are
 * restocked
 * @param uint32 extendedcost : unique cost of an [Item], such as conquest
 * points for example
 */
...
#if defined(ELUNA_SKYFIRE)
  if (!eObjectMgr->IsVendorItemValid(entry, item, maxcount, incrtime,
                                     extendedcost, 1))
#else
  if (!eObjectMgr->IsVendorItemValid(entry, item, maxcount, incrtime,
                                     extendedcost))
...
#if defined(ELUNA_SKYFIRE)
  eObjectMgr->RemoveVendorItem(entry, item, 1, false);
#else
...
#if defined(ELUNA_SKYFIRE)
    eObjectMgr->RemoveVendorItem(entry, (*itr)->item, 1, false);
#else
 ...
 * @param string nameOrIP : If BanMode is 0 then accountname, if 1 then
 * charactername if 2 then ip
 * @param uint32 duration : duration (in seconds) of the ban
 * @param string reason = "" : ban reason, this is optional
 * @param string whoBanned = "" : the [Player]'s name that banned the account,
 * character or IP, this is optional
 * @return int result : status of the ban. 0 if success, 1 if syntax error, 2 if
 * target not found, 3 if a longer ban already exists, nil if unknown result
 */
int Ban(Eluna *E) {
...
#if defined(ELUNA_SKYFIRE)
  {
    std::string upperName = nameOrIP;
    std::transform(upperName.begin(), upperName.end(), upperName.begin(),
                   ::toupper);
    nameOrIP = upperName;
  }
    if (nameOrIP.empty()) // Fallback check
#else
 ...
 * @param [MailStationery] stationary = MAIL_STATIONERY_DEFAULT : type of mail
 * that is being sent as, refer to table above
 ...
 * @return uint32 itemGUIDlow : low GUID of the item. Up to 12 values returned,
 * returns nil if no further items are sent
 */
int SendMail(Eluna *E) {
  ...
  while (addedItems <= MAX_MAIL_ITEMS && i + 2 <= argAmount) {
    ...
    if (amount < 1 ||
        (item_proto->MaxCount > 0 && amount > uint32(item_proto->MaxCount))) {
    ...
    if (Item *item = Item::CreateItem(entry, amount)) {
      item->SaveToDB(trans);
      draft.AddItem(item);
      E->Push(GUID_LOPART(item->GetGUID()));
      ++addedItems;
    }
  }

  Player *receiverPlayer = eObjectAccessor()
#if defined(ELUNA_SKYFIRE)
      FindPlayer(MAKE_NEW_GUID(receiverGUIDLow, 0, HIGHGUID_PLAYER));
#else
      FindPlayer(MAKE_NEW_GUID(receiverGUIDLow, 0, HighGuid::Player));
#endif
  draft.SendMailTo(trans, MailReceiver(receiverPlayer, receiverGUIDLow), sender,
                   MAIL_CHECK_MASK_NONE, delay);
 ...
 *
 * Note that the first taxi point needs to be near the player when he starts the
 * taxi path. The function should also be used only **once** per path added so
 * use it on server startup for example.
 *
 ...
 *
 * @param table waypoints : table containing waypoints: {map, x, y, z[,
 * actionFlag, delay]}
...
int AddTaxiPath(Eluna *E) {
#if defined(ELUNA_SKYFIRE)
  return 0; // Not supported in Skyfire due to lack of SetEntry in DBCStorage
#else
  ...
  while (lua_next(E->L, -2) != 0) {
    ...
    while (lua_next(E->L, -2) != 0) {
      ...
      // key2, value2
      lua_insert(E->L, end++);
      // Stack: {nodes}, mountA, mountH, price, pathid, {nodes}, value2, key,
      // value, key2
    }
    // Stack: {nodes}, mountA, mountH, price, pathid, {nodes}, value2, key,
    // value
    ...
    if (end - start < 4) // no mandatory args, dont add
      return luaL_argerror(E->L, 1,
                           "all waypoints do not have mandatory arguments");

    while (end - start < 8) // fill optional args with 0
    {
      E->Push(0);
      lua_insert(E->L, end++);
      // Stack: {nodes}, mountA, mountH, price, pathid, {nodes}, node, key,
      // value
    }
  ...
  uint32 index = 0;
  for (std::list<TaxiPathNodeEntry>::iterator it = nodes.begin();
       it != nodes.end(); ++it) {
    ...
    entry.path = pathId;
    entry.index = nodeId;
    // nodeEntry->ID = index; // Not in Skyfire
  ...
  if (startNode >= nodeId)
    return 1;
  sTaxiPathSetBySource[startNode][nodeId - 1] =
      TaxiPathBySourceAndDestination(pathId, price);
...
/**
 * Returns `true` if the bag and slot is a valid inventory position, otherwise
 * `false`.
 *
 ...
 * - Slots 0-35
 *
 * @param uint8 bag : the bag the [Item] is in, you can get this with
 * [Item:GetBagSlot]
 * @param uint8 slot : the slot the [Item] is in within the bag, you can get
 * this with [Item:GetSlot]
...
/**
 * Returns `true` if the bag and slot is a valid equipment position, otherwise
 * `false`.
 *
...
/**
 * Returns `true` if the bag and slot is a valid bank position, otherwise
 * `false`.
 *
 * See [Global:IsInventoryPos] for bag/slot combination examples.
 *
...
/**
 * Returns `true` if the bag and slot is a valid bag position, otherwise
 * `false`.
 *
 * See [Global:IsInventoryPos] for bag/slot combination examples.
 *
...
/**
 * Starts the event by eventId, if force is set, the event will force start
 * regardless of previous event state.
 *
...
/**
 * Stops the event by eventId, if force is set, the event will force stop
 * regardless of previous event state.
 *
 ...
 *
 * The value by default is 0, but can be initialized to a value by passing a
 * number or long long as a string.
 *
 * @proto value = ()
 * @proto value = (n)
 * @proto value = (n_ll)
...
int CreateLongLong(Eluna *E) {
  ...
  if (lua_isstring(E->L, 1)) {
    ...
    if (iss.bad())
      return luaL_argerror(E->L, 1,
                           "long long (as string) could not be converted");
 ...
 *
 * The value by default is 0, but can be initialized to a value by passing a
 * number or unsigned long long as a string.
 *
 * @proto value = ()
 * @proto value = (n)
 * @proto value = (n_ull)
  ...
  if (lua_isstring(E->L, 1)) {
    ...
    if (iss.bad())
      return luaL_argerror(
          E->L, 1, "unsigned long long (as string) could not be converted");
  } else if (!lua_isnoneornil(E->L, 1))
...
/**
 * Unbinds event handlers for either all [BattleGround] events, or one type of
 * event.
 *
 ...
 * @proto ()
 * @proto (event_type)
 * @param uint32 event_type : the event whose handlers will be cleared, see
 * [Global:RegisterBGEvent]
 */
...
/**
 * Unbinds event handlers for either all of a [Creature]'s events, or one type
 * of event.
 *
 ...
 * To bind and unbind events to a single [Creature], see
 * [Global:RegisterUniqueCreatureEvent] and [Global:ClearUniqueCreatureEvents].
 *
 * @proto (entry)
 * @proto (entry, event_type)
 * @param uint32 entry : the ID of one or more [Creature]s whose handlers will
 * be cleared
 * @param uint32 event_type : the event whose handlers will be cleared, see
 * [Global:RegisterCreatureEvent]
 */
 ...
 * To bind and unbind events to all instances of a [Creature], see
 * [Global:RegisterCreatureEvent] and [Global:ClearCreatureEvent].
 *
 * @proto (entry)
 * @proto (entry, event_type)
 * @param ObjectGuid guid : the GUID of a single [Creature] whose handlers will
 * be cleared
 * @param uint32 instance_id : the instance ID of a single [Creature] whose
...
/**
 * Unbinds event handlers for either all of a [Creature]'s gossip events, or one
 * type of event.
 *
 * If `event_type` is `nil`, all the [Creature]'s gossip event handlers are
 * cleared.
 *
 * Otherwise, only event handlers for `event_type` are cleared.
 *
 * **NOTE:** this will affect all instances of the [Creature], not just one.
 * To bind and unbind gossip events to a single [Creature], tell the Eluna
 * developers to implement that.
 *
 * @proto (entry)
 * @proto (entry, event_type)
 * @param uint32 entry : the ID of a [Creature] whose handlers will be cleared
 * @param uint32 event_type : the event whose handlers will be cleared, see
 * [Global:RegisterCreatureGossipEvent]
 */
...
/**
 * Unbinds event handlers for either all of a [GameObject]'s events, or one type
 * of event.
 *
 ...
 * To bind and unbind events to a single [GameObject], tell the Eluna developers
 * to implement that.
 *
 * @proto (entry)
 * @proto (entry, event_type)
 ...
 * @param uint32 event_type : the event whose handlers will be cleared, see
 * [Global:RegisterGameObjectEvent]
 */
...
/**
 * Unbinds event handlers for either all of a [GameObject]'s gossip events, or
 * one type of event.
 *
 * If `event_type` is `nil`, all the [GameObject]'s gossip event handlers are
 * cleared.
 *
 * Otherwise, only event handlers for `event_type` are cleared.
 *
 * **NOTE:** this will affect all instances of the [GameObject], not just one.
 * To bind and unbind gossip events to a single [GameObject], tell the Eluna
 * developers to implement that.
 *
 * @proto (entry)
 * @proto (entry, event_type)
 * @param uint32 entry : the ID of a [GameObject] whose handlers will be cleared
 * @param uint32 event_type : the event whose handlers will be cleared, see
 * [Global:RegisterGameObjectGossipEvent]
 */
...
/**
 * Unbinds event handlers for either all [Group] events, or one type of [Group]
 * event.
 *
...
/**
 * Unbinds event handlers for either all [Guild] events, or one type of [Guild]
 * event.
 *
...
/**
 * Unbinds event handlers for either all of an [Item]'s events, or one type of
 * event.
 *
 ...
 * To bind and unbind events to a single [Item], tell the Eluna developers to
 * implement that.
 *
 * @proto (entry)
 * @proto (entry, event_type)
...
/**
 * Unbinds event handlers for either all of an [Item]'s gossip events, or one
 * type of event.
 *
 ...
 *
 * **NOTE:** this will affect all instances of the [Item], not just one.
 * To bind and unbind gossip events to a single [Item], tell the Eluna
 * developers to implement that.
 *
 * @proto (entry)
 * @proto (entry, event_type)
 * @param uint32 entry : the ID of an [Item] whose handlers will be cleared
 * @param uint32 event_type : the event whose handlers will be cleared, see
 * [Global:RegisterItemGossipEvent]
 */
...
/**
 * Unbinds event handlers for either all of a [WorldPacket] opcode's events, or
 * one type of event.
 *
 * If `event_type` is `nil`, all the [WorldPacket] opcode's event handlers are
 * cleared.
 *
 * Otherwise, only event handlers for `event_type` are cleared.
 *
 ...
 * @param uint32 opcode : the type of [WorldPacket] whose handlers will be
 * cleared
 * @param uint32 event_type : the event whose handlers will be cleared, see
 * [Global:RegisterPacketEvent]
 */
...
/**
 * Unbinds event handlers for either all [Player] events, or one type of
 * [Player] event.
 *
...
/**
 * Unbinds event handlers for either all of a [Player]'s gossip events, or one
 * type of event.
 *
 * If `event_type` is `nil`, all the [Player]'s gossip event handlers are
 * cleared.
 *
 * Otherwise, only event handlers for `event_type` are cleared.
 *
 * @proto (entry)
 * @proto (entry, event_type)
 * @param uint32 entry : the low GUID of a [Player] whose handlers will be
 * cleared
 * @param uint32 event_type : the event whose handlers will be cleared, see
 * [Global:RegisterPlayerGossipEvent]
 */
...
/**
 * Unbinds event handlers for either all of a non-instanced [Map]'s events, or
 * one type of event.
 *
 * If `event_type` is `nil`, all the non-instanced [Map]'s event handlers are
 * cleared.
 *
 * Otherwise, only event handlers for `event_type` are cleared.
 *
...
/**
 * Unbinds event handlers for either all of an instanced [Map]'s events, or one
 * type of event.
 *
 * If `event_type` is `nil`, all the instanced [Map]'s event handlers are
 * cleared.
 *
 * Otherwise, only event handlers for `event_type` are cleared.
 *
...
ElunaRegister<> GlobalMethods[] = {
    ...
    {"RegisterUniqueCreatureEvent",
     &LuaGlobalFunctions::RegisterUniqueCreatureEvent},
    {"RegisterCreatureGossipEvent",
     &LuaGlobalFunctions::RegisterCreatureGossipEvent},
    {"RegisterGameObjectEvent", &LuaGlobalFunctions::RegisterGameObjectEvent},
    {"RegisterGameObjectGossipEvent",
     &LuaGlobalFunctions::RegisterGameObjectGossipEvent},
    ...
    {"ClearUniqueCreatureEvents",
     &LuaGlobalFunctions::ClearUniqueCreatureEvents},
    {"ClearCreatureGossipEvents",
     &LuaGlobalFunctions::ClearCreatureGossipEvents},
    ...
    {"GetStateMap", &LuaGlobalFunctions::GetStateMap,
     METHOD_REG_MAP}, // Map state method only in multistate
    ...
    {"GetPlayerByGUID", &LuaGlobalFunctions::GetPlayerByGUID,
     METHOD_REG_WORLD}, // World state method only in multistate
    ...
    {"GetPlayersInWorld", &LuaGlobalFunctions::GetPlayersInWorld,
     METHOD_REG_WORLD}, // World state method only in multistate
    {"GetPlayersOnMap", &LuaGlobalFunctions::GetPlayersOnMap,
     METHOD_REG_MAP}, // Map state method only in multistate
    ...
    {"CharDBQuery", &LuaGlobalFunctions::CharDBQuery, METHOD_REG_ALL,
     METHOD_FLAG_UNSAFE},
    {"CharDBExecute", &LuaGlobalFunctions::CharDBExecute},
    {"CharDBQueryAsync", &LuaGlobalFunctions::CharDBQueryAsync},
    {"AuthDBQuery", &LuaGlobalFunctions::AuthDBQuery, METHOD_REG_ALL,
     METHOD_FLAG_UNSAFE},
