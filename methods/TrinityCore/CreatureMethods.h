...
int IsRegeneratingHealth(Eluna *E, Creature *creature) {
  // E->Push(creature->CanRegenerateHealth()); // FIXME: Skyfire MoP equivalent
  // missing
 ...
 *
 * @param bool enable = true : `true` to enable health regeneration, `false` to
 * disable it
 */
 ...
 *   and returns `false` otherwise.
 *
 * @param bool mustBeDead = false : if `true`, only returns `true` if the
 * [Creature] is also dead. Otherwise, it must be alive.
...
/**
 * Returns `true` if the [Creature] can assist `friend` in combat against
 * `enemy`, and returns `false` otherwise.
 *
 * @param [Unit] friend : the Unit we will be assisting
 * @param [Unit] enemy : the Unit that we would attack if we assist `friend`
 * @param bool checkFaction = true : if `true`, the [Creature] must be the same
 * faction as `friend` to assist
...
/**
 * Returns `true` if the [Creature] is returning to its spawn position from
 * combat, and returns `false` otherwise.
 *
...
/**
 * Returns `true` if the [Creature]'s flags_extra includes Dungeon Boss
 * (0x1000000), and returns `false` otherwise.
 *
...
/**
 * Returns `true` if the [Creature] cannot cast `spellId` due to a category
 * cooldown, and returns `false` otherwise.
 *
...
int GetCurrentWaypointId(Eluna *E, Creature *creature) {
  E->Push(creature->GetCurrentWaypointID());
  return 1;
}
...
/**
 * Returns the delay between when the [Creature] dies and when its body
 * despawns.
 *
 ...
 * @values [SELECT_TARGET_BOTTOMAGGRO, 2, "Sorts targets from bottom aggro to
 * top"]
 * @values [SELECT_TARGET_NEAREST, 3, "Sorts targets from the nearest to the
 * furthest"]
 * @values [SELECT_TARGET_FARTHEST, 4, "Sorts targets from the furthest to the
 * nearest"]
 *
 ...
 * @param bool playerOnly = false : if `true`, skips targets that aren't
 * [Player]s
 * @param uint32 position = 0 : used as an offset into the threat list. If
 * `targetType` is random, used as the number of players from top of aggro to
 * choose from
 * @param float distance = 0.0 : if positive, the maximum distance for the
 * target. If negative, the minimum distance
 * @param int32 aura = 0 : if positive, the target must have this [Aura]. If
 * negative, the the target must not have this Aura
...
int GetAITarget(Eluna *E, Creature *creature) {
  ...
  for (HostileReference const *itr : threatlist) {
    ...
    if (!target)
      continue;
    if (playerOnly && target->GetTypeId() != TypeID::TYPEID_PLAYER)
      continue;
...
int GetAITargets(Eluna *E, Creature *creature) {
  ThreatContainer::StorageType const &threatlist =
      creature->getThreatManager().getThreatList();
 ...
 * @param [SpellSchoolMask] schoolMask = 0 : [SpellSchoolMask] of the threat
 * causer
...
int AddThreat(Eluna *E, Creature *creature) {
  ...
  creature->getThreatManager().addThreat(
      victim, threat, SPELL_SCHOOL_MASK_NORMAL,
      spell ? sSpellMgr->GetSpellInfo(spell) : NULL);
  return 0;
}
...
int ClearThreat(Eluna *E, Creature *creature) {
  ...
  creature->getThreatManager().addThreat(
      target, -creature->getThreatManager().getThreat(target));
  return 0;
}
...
/**
 * Forces the [Creature] to fixate on the [Unit], regardless of threat. Requires
 * the [Unit] to be in the threat list.
 *
 * @param [Unit] target
 */
...
int ClearFixate(Eluna * /*E*/, Creature * /*creature*/) {
  // creature->getThreatManager().removeFixate(); // Not in MoP ThreatManager
  return 0;
}
...
int GetNPCFlags(Eluna *E, Creature *creature) {
  E->Push(creature->GetUInt32Value(UNIT_FIELD_NPC_FLAGS));
  return 1;
}
...
int GetShieldBlockValue(Eluna *E, Creature *creature) {
  E->Push(creature->GetBlockPercent()); // Fallback if ShieldBlockValue is
                                        // missing in MoP or changed name
  return 1;
}
...
int GetDBTableGUIDLow(Eluna *E, Creature *creature) {
  E->Push(creature->GetDBTableGUIDLow());
  return 1;
}
...
int SetNPCFlags(Eluna *E, Creature *creature) {
  ...
  creature->SetUInt32Value(UNIT_FIELD_NPC_FLAGS, flags);
  return 0;
}
...
int SetEquipmentSlots(Eluna *E, Creature *creature) {
  ...
  creature->SetUInt32Value(UNIT_FIELD_VIRTUAL_ITEM_ID + 0, main_hand);
  creature->SetUInt32Value(UNIT_FIELD_VIRTUAL_ITEM_ID + 1, off_hand);
  creature->SetUInt32Value(UNIT_FIELD_VIRTUAL_ITEM_ID + 2, ranged);
  return 0;
}
...
/**
 * Sets whether the [Creature] can call nearby enemies for help in combat or
 * not.
 *
 * @param bool enable = true : `true` to disable calling for help, `false` to
 * enable
 */
...
/**
 * Make the [Creature] call for assistance in combat from other nearby
 * [Creature]s.
 */
...
/**
 * Make the [Creature] call for help in combat from friendly [Creature]s within
 * `radius`.
 *
 * @param float radius
 */
...
/**
 * Make the [Creature] flee combat to get assistance from a nearby friendly
 * [Creature].
 */
 ...
 * @param uint32 dataGUIDLow = 0 : use this Creature's model and equipment
...
int UpdateEntry(Eluna *E, Creature *creature) {
  ...
  creature->UpdateEntry(entry, ALLIANCE,
                        dataGuidLow ? eObjectMgr->GetCreatureData(dataGuidLow)
                                    : NULL);
  return 0;
}
...
/**
 * Returns the [Creature]'s creature family ID (enumerated in
 * CreatureFamily.dbc).
 *
 * @table
 ...
 * @values [CREATURE_FAMILY_HORSE_CUSTOM, 10, "Does not exist in DBC but used
 * for horse like beasts in DB"]
 ...
 * @values [CREATURE_FAMILY_BIRD_OF_PREY, 26, "Named CREATURE_FAMILY_OWL in
 * Mangos"]
 ...
 *
 * @param bool deleteFromDB : if true, it will delete the [Creature] from the
 * database
 */
...
ElunaRegister<Creature> CreatureMethods[] = {
    ...
    {"GetCreatureSpellCooldownDelay",
     &LuaCreature::GetCreatureSpellCooldownDelay},
    ...
    {"IsDamageEnoughForLootingAndReward",
     &LuaCreature::IsDamageEnoughForLootingAndReward},
