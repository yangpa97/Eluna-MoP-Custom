...
int GetWorldObject(Eluna *E, Map *map) {
  ...
  switch (GUID_HIPART((uint64)guid)) // Use GUID_HIPART macro
  {
  ...
  case HIGHGUID_UNIT:
    ...
    break;
  // case HIGHGUID_PET:
  //    E->Push(map->GetPet(guid)); // Not supported
  //    break;
  case HIGHGUID_DYNAMICOBJECT:
    E->Push(map->GetDynamicObject(guid));
    break;
  // case HIGHGUID_CORPSE:
  //    E->Push(map->GetCorpse(guid)); // Not supported
  //    break;
  default:
    break;
  }
 ...
 * @param [WeatherType] type : the [WeatherType], see above available weather
 * types
 ...
 * @values [NEUTRAL, 2]
 *
 * @param [TeamId] team : optional check team of the [Player], Alliance, Horde
 * or Neutral (All)
 * @return table mapPlayers
 */
int GetPlayers(Eluna *E, Map *map) {
  uint32 team = E->CHECKVAL<uint32>(2, 2 /*TEAM_NEUTRAL*/);
  ...
  for (Map::PlayerList::const_iterator itr = players.begin();
    ...
    if (player->GetSession() &&
        (team >= 2 /*TEAM_NEUTRAL*/ || uint32(player->GetTeamId()) == team)) {
      E->Push(player);
 ...
 * This data will remain for as long as the [Map] exists, or until a server
 * restart.
 *
