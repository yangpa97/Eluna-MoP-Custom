...
/**
 * Returns 'true' if the [Item] is bound to a [Player] by an enchant, 'false'
 * otehrwise
 *
...
/**
 * Returns 'true' if the [Item] is not bound to the [Player] specified, 'false'
 * otherwise
 *
...
/**
 * Returns 'true' if the [Item] is currently in a trade window, 'false'
 * otherwise
 *
...
/**
 * Returns 'true' if the [Item] has the [Quest] specified tied to it, 'false'
 * otherwise
 *
...
/**
 * Returns 'true' if the refund period has expired for this [Item], 'false'
 * otherwise
 *
 ...
 *
 * @param [LocaleConstant] locale = DEFAULT_LOCALE : locale to return the
 * [Item]'s name in
...
int GetItemLink(Eluna *E, Item *item) {
  ...
  if (int32 itemRandPropId = item->GetItemRandomPropertyId()) {
    ...
    if (itemRandPropId < 0) {
      const ItemRandomSuffixEntry *itemRandEntry =
          sItemRandomSuffixStore.LookupEntry(-item->GetItemRandomPropertyId());
      if (itemRandEntry)
        suffix = &itemRandEntry->nameSuffix;
    } else {
      const ItemRandomPropertiesEntry *itemRandEntry =
  ...
  oss << "|c" << std::hex << ItemQualityColors[temp->Quality] << std::dec
      ...
      << ":" << (uint32)item->GetOwner()->getLevel() << "|h[" << name
      << "]|h|r";
...
int GetDamageInfo(Eluna *E, Item *item) {
  ...
  if (damageSlot == 1) { // SkyFire main damage
    damageType = item->GetTemplate()->DamageType;
    damageMin = item->GetTemplate()->DamageMin;
    damageMax = item->GetTemplate()->DamageMax;
  }
...
int GetArmor(Eluna *E, Item *item) {
  E->Push(0); // Armor not directly available in SkyFire ItemTemplate
  return 1;
}
...
int GetMaxDurability(Eluna *E, Item *item) {
  E->Push(item->GetUInt32Value(ITEM_FIELD_MAX_DURABILITY));
  return 1;
}
 ...
 * @return bool enchantmentSuccess : if enchantment is successfully set to
 * specified slot, returns 'true', otherwise 'false'
 */
 ...
 * @return bool enchantmentRemoved : if enchantment is successfully removed from
 * specified slot, returns 'true', otherwise 'false'
 */
...
int SaveToDB(Eluna * /*E*/, Item *item) {
  ...
  // item->SaveToDB(trans);
  // SkyFire Item::SaveToDB takes SQLTransaction&, but usually we don't start
  // transaction manually for single item save unless needed. However, looking
  // at SkyFire source, Item::SaveToDB(SQLTransaction& trans) exists. We can
  // pass a null transaction if supported or we need to start one. But wait,
  // Item::SaveToDB signature in Item.h is: virtual void
  // SaveToDB(SQLTransaction& trans); We need a valid transaction.
  SQLTransaction trans = CharacterDatabase.BeginTransaction();
  item->SaveToDB(trans);
