...
namespace LuaBigInt {
template <typename T, typename Op> constexpr int PerformOp(Eluna *E, Op op) {
...
template <typename T> int Add(Eluna *E, T *) {
  return PerformOp<T>(E, std::plus<T>{});
}

template <typename T> int Subtract(Eluna *E, T *) {
  return PerformOp<T>(E, std::minus<T>{});
}

template <typename T> int Multiply(Eluna *E, T *) {
  return PerformOp<T>(E, std::multiplies<T>{});
}

template <typename T> int Divide(Eluna *E, T *) {
  return PerformOp<T>(E, std::divides<T>{});
}

template <typename T> int Mod(Eluna *E, T *) {
  return PerformOp<T>(E, std::modulus<T>{});
}

template <typename T> int UnaryMinus(Eluna *E, T *) {
  T val = E->CHECKVAL<T>(1);
  E->Push(std::negate<T>{}(val));
  return 1;
}

template <typename T> int Equal(Eluna *E, T *) {
  return PerformOp<T>(E, std::equal_to<T>{});
}

template <typename T> int Less(Eluna *E, T *) {
  return PerformOp<T>(E, std::less<T>{});
}

template <typename T> int LessOrEqual(Eluna *E, T *) {
  return PerformOp<T>(E, std::less_equal<T>{});
}

template <typename T> int ToString(Eluna *E, T *) {
  T val = E->CHECKVAL<T>(1);
...
template <typename T> int Pow(Eluna *E, T *) {
  T val1 = E->CHECKVAL<T>(1);
  T val2 = E->CHECKVAL<T>(2);
  E->Push(static_cast<T>(
      powl(static_cast<long double>(val1), static_cast<long double>(val2))));
  return 1;
}
...
int ToString(Eluna *E, ObjectGuid *) {
  E->Push(std::to_string((uint64)E->CHECKVAL<ObjectGuid>(1)));
  return 1;
}
