...
namespace LuaPlayer {
/**
 * Returns 'true' if the [Player] can Titan Grip the specific [Item], 'false'
 * otherwise.
...
/**
 * Returns 'true' if the [Player] has a talent by ID in specified spec, 'false'
 * otherwise.
 *
...
/**
 * Returns 'true' if the [Player] has completed the specified achievement,
 * 'false' otherwise.
 *
...
/**
 * Returns 'true' if the [Player] has an active [Quest] by specific ID, 'false'
 * otherwise.
 *
...
/**
 * Returns 'true' if the [Player] has a [Spell] by specific ID, 'false'
 * otherwise.
 *
...
/**
 * Returns 'true' if the [Player] has the given amount of item entry specified,
 * 'false' otherwise.
 *
...
/**
 * Returns 'true' if the [Player] has a quest for the item entry specified,
 * 'false' otherwise.
 *
...
/**
 * Returns 'true' if the [Player] can use the item or item entry specified,
 * 'false' otherwise.
 *
 * @proto canUse = (item)
 * @proto canUse = (entry)
...
/**
 * Returns 'true' if the [Spell] specified by ID is currently on cooldown for
...
/**
 * Returns 'true' if the [Player] can share [Quest] specified by ID, 'false'
 * otherwise.
 *
...
/**
 * Returns 'true' if the [Player] can currently communicate through chat,
 * 'false' otherwise.
 *
 * @return bool canSpeak
 */
int CanSpeak(Eluna *E, Player *player) {
  E->Push(player->GetSession()->m_muteTime <= sWorld->GetGameTime());
  return 1;
}

/**
 * Returns 'true' if the [Player] has permission to uninvite others from the
 * current group, 'false' otherwise.
 *
 * @return bool canUninviteFromGroup
 */
int CanUninviteFromGroup(Eluna *E, Player *player) {
  E->Push(player->CanUninviteFromGroup() == PartyResult::ERR_PARTY_RESULT_OK);
  return 1;
}
 ...
 *
 * Note: This is only true when GM tag is activated! For alternative see
 * [Player:GetGMRank]
 *
 * @return bool isGM
 */
...
/**
 * Returns 'true' if the [Player] is in an arena team specified by type, 'false'
 * otherwise.
 *
 * @param uint32 type
...
/**
 * Returns 'true' if the [Player] satisfies all requirements to complete the
 * quest entry.
 *
 * @param uint32 entry
...
/**
 * Returns 'true' if the [Player] satisfies all requirements to turn in the
 * quest.
 *
 * @param uint32 entry
...
/**
 * Returns 'true' if the [Player] is a part of the Horde faction, 'false'
 * otherwise.
 *
 * @return bool isHorde
 */
...
/**
 * Returns 'true' if the [Player] is a part of the Alliance faction, 'false'
 * otherwise.
 *
...
/**
 * Returns 'true' if the [Player] is 'Do Not Disturb' flagged, 'false'
 * otherwise.
 *
 * @return bool isDND
 */
...
/**
 * Returns 'true' if the [Player] is 'Away From Keyboard' flagged, 'false'
 * otherwise.
 *
 * @return bool isAFK
 */
...
/**
 * Returns whether or not the [Player]s [Group] is visible for the other
 * specific [Player].
 *
 * @param [Player] player
...
/**
 * Returns 'true' if the [Player] is currently in the same raid as another
 * [Player] by object, 'false' otherwise.
 *
 * @param [Player] player
...
/**
 * Returns 'true' if the [Player] is currently in the same [Group] as another
 * [Player] by object, 'false' otherwise.
 *
 * @param [Player] player
...
/**
 * Returns 'true' if the [Player] is eligible for Honor or XP gain by [Unit]
...
/**
 * Returns 'true' if the [Player] can see anoter [Player] specified by object,
 * 'false' otherwise.
 *
 * @param [Player] player
...
/**
 * Returns 'true' if the [Player] is currently in a [BattleGround] queue,
 * 'false' otherwise.
 *
...
/**
 * Returns 'true' if the [Player] is currently in a [BattleGround], 'false'
 * otherwise.
 *
...
/**
 * Returns 'true' if the [Player] can block incomming attacks, 'false'
 * otherwise.
 *
...
/**
 * Returns 'true' if the [Player] can parry incomming attacks, 'false'
 * otherwise.
 *
...
/**
 * Returns whether or not the [Player] has received the reward for a specific
 * [Quest] ID
 *
 * @param uint32 questId
...
/**
 * Returns whether or not the [Player] is currently immune to environmental
 * damage
 *
 ...
 * @return bool hasRecruited : returns 'true' if the [Player] recruited the
 * other [Player], false otherwise
 */
int HasRecruited(Eluna *E, Player *player) {
  Player *recruit = E->CHECKOBJ<Player>(2);
  E->Push(recruit->GetSession()->GetRecruiterId() ==
...
/**
 * Returns the normal phase of the player instead of the actual phase possibly
 * containing GM phase
 *
 * @return uint32 phasemask
 */
int GetPhaseMaskForSpawn(Eluna *E, Player *player) {
  E->Push(player->GetPhaseMask());
  return 1;
}
...
int GetArenaPoints(Eluna *E, Player *player) {
  E->Push(player->GetCurrency(390, false));
  return 1;
}
...
int GetHonorPoints(Eluna *E, Player *player) {
  E->Push(player->GetCurrency(392, false));
  return 1;
}
...
int GetShieldBlockValue(Eluna *E, Player *player) {
  E->Push((uint32)player->GetTotalBaseModValue(SHIELD_BLOCK_VALUE));
  return 1;
}
...
int GetSpellCooldownDelay(Eluna *E, Player *player) {
  ...
  if (SpellInfo const *spellInfo = sSpellMgr->GetSpellInfo(spellId))
    E->Push(player->GetSpellCooldownDelay(spellId));
  else
    E->Push(0);
...
/**
 * Returns a random Raid Member [Player] object within radius specified of
 * [Player]
 *
 * @param float radius
...
int GetBattlegroundTypeId(Eluna *E, Player *player) {
  E->Push((uint32)player->GetBattlegroundTypeId());
  return 1;
}
...
int GetBattlegroundId(Eluna *E, Player *player) {
  E->Push((uint32)player->GetBattlegroundId());
  return 1;
}
...
int GetManaBonusFromIntellect(Eluna *E, Player *player) {
  // Mana is fixed per level in MoP 5.4.8
  E->Push(0.0f);
  return 1;
}
...
int GetHealthBonusFromStamina(Eluna *E, Player *player) {
  // Use explicit Player:: to avoid any visibility issues if reported missing
  E->Push(player->Player::GetHealthBonusFromStamina());
  return 1;
}
...
int GetDifficulty(Eluna *E, Player *player) {
  ...
  E->Push((uint32)(isRaid ? player->GetRaidDifficulty()
...
int GetFreeTalentPoints(Eluna *E, Player *player) {
  E->Push(player->GetUInt32Value(PLAYER_FIELD_CHARACTER_POINTS));
  return 1;
}
...
/**
 * Returns 'true' if the [Player]s [Quest] specified by entry ID has been
 * rewarded, 'false' otherwise.
 *
 * @param uint32 questId
...
int GetMailItem(Eluna *E, Player *player) {
  ...
  E->Push(player->GetMItem((uint32)(uint64)guid));
  return 1;
}
...
/**
 * Returns the database textID of the [WorldObject]'s gossip header text for the
 * [Player]
 *
...
int GetGMRank(Eluna *E, Player *player) {
  E->Push(static_cast<uint32>(player->GetSession()->GetSecurity()));
  return 1;
}
 ...
 *
 * @param uint32 entry : entry of the item
 * @param bool checkinBank = false : also counts the items in player's bank if
 * true
...
int GetXP(Eluna *E, Player *player) {
  E->Push(player->GetUInt32Value(PLAYER_FIELD_XP));
  return 1;
}
...
int GetXPForNextLevel(Eluna *E, Player *player) {
  E->Push(player->GetUInt32Value(PLAYER_FIELD_NEXT_LEVEL_XP));
  return 1;
}
...
int SetPlayerLock(Eluna *E, Player *player) {
  ...
  if (apply) {
    ...
    player->RemoveFlag(UNIT_FIELD_FLAGS,
                       UNIT_FLAG_PACIFIED | UNIT_FLAG_SILENCED);
...
/**
 * Sets the [Player]s free talent points to the amount specified for the current
 * spec
 *
...
int SetFreeTalentPoints(Eluna *E, Player *player) {
  ...
  player->SetUInt32Value(PLAYER_FIELD_CHARACTER_POINTS, points);
...
int SetCoinage(Eluna *E, Player *player) {
  uint64 amt = E->CHECKVAL<uint64>(2);
  player->SetMoney(amt);
  return 0;
}
...
int SendShowMailBox(Eluna *E, Player *player) {
  // ObjectGuid guid = E->CHECKVAL<uint64>(2, player->GET_GUID());
  // Skyfire doesn't seem to have a simple SendShowMailBox method on
  // WorldSession
  // TODO: Find correct packet sequence for Skyfire MoP 5.4.8
  return 0;
}
...
int ModifyArenaPoints(Eluna *E, Player *player) {
  ...
  player->ModifyCurrency(395, amount);
  return 0;
}
...
int ModifyHonorPoints(Eluna *E, Player *player) {
  ...
  player->ModifyCurrency(392, amount);
  return 0;
}
...
int Mute(Eluna *E, Player *player) {
  ...
  LoginDatabase.PExecute("UPDATE account SET mutetime = {} WHERE id = {}",
                         muteTime, player->GetSession()->GetAccountId());
  return 0;
}
...
int SendAuctionMenu(Eluna *E, Player *player) {
  ...
  player->GetSession()->SendAuctionHello(unit->GET_GUID(), (Creature *)unit);
  return 0;
}
...
/**
 * Sends a guild invitation from the [Player]s [Guild] to the [Player] object
 * specified
 *
...
int UnbindAllInstances(Eluna * /*E*/, Player *player) {
  for (uint8 i = 0; i < 15; ++i) {
    Player::BoundInstancesMap &binds =
        player->GetBoundInstances(DifficultyID(i));
    for (Player::BoundInstancesMap::iterator itr = binds.begin();
         itr != binds.end();) {
...
int ResetTalentsCost(Eluna *E, Player *player) {
  E->Push(player->GetTalentResetCost());
  return 1;
}
...
/**
 * Completes the given quest entry for the [Player] and tries to satisfy all
 * quest requirements.
 *
...
int CompleteQuest(Eluna *E, Player *player) {
  ...
  for (uint8 x = 0; x < QUEST_SOURCE_ITEM_IDS_COUNT; ++x) {
    uint32 id = quest->RequiredSourceItemId[x];
    uint32 count = quest->RequiredSourceItemCount[x];
    ...
    uint8 msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, id,
                                        count - curItemCount);
    if (msg == EQUIP_ERR_OK) {
  ...
  // All creature/GO slain/cast (not required, but otherwise it will display
  // "Creature slain 0/10")
  for (auto const *obj : quest->m_questObjectives) {
    if (obj->Type == QUEST_OBJECTIVE_TYPE_NPC ||
        obj->Type == QUEST_OBJECTIVE_TYPE_GO) {
      int32 creature = (int32)obj->ObjectId;
      uint32 creatureCount = obj->Amount;

      if (creature > 0) {
        if (CreatureTemplate const *creatureInfo =
                sObjectMgr->GetCreatureTemplate(creature))
  ...
  // Reputation requirements mapping for MoP
  for (uint8 i = 0; i < QUEST_REPUTATIONS_COUNT; ++i) {
    if (uint32 repFaction = quest->RewardFactionId[i]) {
      int32 repValue = quest->RewardFactionValueId[i];
      uint32 curRep = player->GetReputationMgr().GetReputation(repFaction);
      if (curRep < (uint32)repValue)
  ...
  int32 ReqOrRewMoney = quest->GetRewMoney();
  if (ReqOrRewMoney < 0)
...
int AddQuest(Eluna *E, Player *player) {
  ...
  // check item starting quest (it can work incorrectly if added without item in
  // inventory)
  ItemTemplateContainer const *itc = sObjectMgr->GetItemTemplateStore();
  auto itr =
      std::find_if(itc->begin(), itc->end(),
                   [quest](ItemTemplateContainer::value_type const &value) {
  ...
  for (uint8 slot = 0; slot < MAX_QUEST_LOG_SIZE; ++slot) {
    ...
    if (logQuest == entry) {
      ...
      if (quest->HasFlag(QUEST_FLAGS_FLAGS_PVP)) {
        player->pvpInfo.IsHostile =
            player->pvpInfo.IsInHostileArea || player->HasPvPForcingQuest();
 ...
 * @param [Player] receiver : is the [Player] that will receive the whisper, if
 * TrinityCore
 * @param ObjectGuid guid : is the GUID of a [Player] that will receive the
 * whisper, not TrinityCore
 */
int Whisper(Eluna *E, Player *player) {
  ...
  player->Whisper(text, (Language)lang, receiver->GET_GUID());
  return 0;
}
...
/**
 * Equips the given item or item entry to the given slot. Returns the equipped
 * item or nil.
 *
 * @table
 ...
 * @param uint32 slot : equipment slot to equip the item to The slot can be
...
int EquipItem(Eluna *E, Player *player) {
  ...
  if (!item) {
    ...
    player->UpdateAchievementCriteria(
        ACHIEVEMENT_CRITERIA_TYPE_RECEIVE_EPIC_ITEM, entry, 1);
  } else {
    InventoryResult result = player->CanEquipItem(slot, dest, item, false);
    if (result != EQUIP_ERR_OK)
      return 1;
...
/**
 * Returns true if the player can equip the given [Item] or item entry to the
  ...
  for (uint32 i = 0; i < sSkillLineStore.GetNumRows(); ++i) {
    if (SkillLineEntry const *entry = sSkillLineStore.LookupEntry(i)) {
      if (entry->categoryId == SKILL_CATEGORY_LANGUAGES ||
          entry->categoryId == SKILL_CATEGORY_GENERIC)
        continue;
...
int Teleport(Eluna *E, Player *player) {
  ...
  //   player->FinishTaxiFlight(); // FIXME: Skyfire MoP equivalent missing
  // else
...
int AddLifetimeKills(Eluna *E, Player *player) {
  ...
  uint32 currentKills =
      player->GetUInt32Value(PLAYER_FIELD_LIFETIME_HONORABLE_KILLS);
  uint32 kills = static_cast<uint32>(
      (static_cast<int32>(currentKills) + val) < 0 ? 0 : currentKills + val);
...
int AddItem(Eluna *E, Player *player) {
  ...
  InventoryResult msg = player->CanStoreNewItem(
      NULL_BAG, NULL_SLOT, dest, itemId, itemCount, &noSpaceForCount);
  if (msg != EQUIP_ERR_OK)
...
int ResetTypeCooldowns(Eluna *E, Player *player) {
  ...
  bool update = E->CHECKVAL<bool>(3, true);
  (void)update; // ensure that the variable is referenced in order to pass
                // compiler checks
...
int SendAddonMessage(Eluna *E, Player *player) {
  ...
  WorldPacket data;
  ChatHandler::BuildChatPacket(data, channel, Language::LANG_ADDON, player,
                               receiver, fullmsg);
...
int LearnTalent(Eluna *E, Player *player) {
  ...
  player->LearnTalent(id);
  player->SendTalentsInfoData();
  return 0;
}

/**
 * Remove cooldowns on spells that have less than 10 minutes of cooldown from
 * the [Player], similarly to when you enter an arena.
 */
...
/**
 * Adds a new item to the gossip menu shown to the [Player] on next call to
 * [Player:GossipSendMenu].
 *
 * sender and intid are numbers which are passed directly to the gossip
 * selection handler. Internally they are partly used for the database gossip
 * handling.<br /> code specifies whether to show a box to insert text to. The
 * player inserted text is passed to the gossip selection handler.<br /> money
 * specifies an amount of money the player needs to have to click the option. An
 * error message is shown if the player doesn't have enough money.<br /> Note
 * that the money amount is only checked client side and is not removed from the
 * player either. You will need to check again in your code before taking
 * action.
 *
 * See also: [Player:GossipSendMenu], [Player:GossipAddQuests],
 * [Player:GossipComplete], [Player:GossipClearMenu]
 *
 ...
 * @param string popup = nil : if non empty string, a popup with given text
 * shown on click
...
int GossipMenuAddItem(Eluna *E, Player *player) {
  ...
  player->PlayerTalkClass->GetGossipMenu().AddMenuItem(
      -1, GossipOptionIcon(_icon), msg, _sender, _intid, _promptMsg, _money,
      _code);
  return 0;
}
 ...
 * [Player:GossipSendMenu], [Player:GossipClearMenu]
 */
...
/**
 * Sends the current gossip items of the player to him as a gossip menu with
 * header text from the given textId.
 *
 * If sender is a [Player] then menu_id is mandatory, otherwise it is not used
 * for anything. menu_id is the ID used to trigger the OnGossipSelect registered
 * for players. See [Global:RegisterPlayerGossipEvent]
 *
 * See also: [Player:GossipMenuAddItem], [Player:GossipAddQuests],
 * [Player:GossipComplete], [Player:GossipClearMenu]
 *
 ...
 * @param uint32 npc_text : entry ID of a header text in npc_text database
 * table, common default is 100
...
int GossipSendMenu(Eluna *E, Player *player) {
  ...
  if (sender->GetTypeId() == TypeID::TYPEID_PLAYER) {
 ...
 *
 * See also: [Player:GossipMenuAddItem], [Player:GossipSendMenu],
 * [Player:GossipAddQuests], [Player:GossipComplete]
 *
 *     Note: This is needed when you show a gossip menu without using gossip
 * hello or select hooks which do this automatically. Usually this is needed
 * when using [Player] is the sender of a Gossip Menu.
 */
...
int GossipSendPOI(Eluna *E, Player *player) {
  ...
  WorldPacket pkt(SMSG_GOSSIP_POI, 4 + 4 + 4 + 4 + 4 + 10);
  pkt << uint32(flags);
  pkt << float(x);
  pkt << float(y);
  pkt << uint32(icon);
...
/**
 * Adds the gossip items to the [Player]'s gossip for the quests the given
 * [WorldObject] can offer to the player.
 *
...
int GossipAddQuests(Eluna *E, Player *player) {
  ...
  if (source->GetTypeId() == TypeID::TYPEID_UNIT) {
    if (source->GetUInt32Value(UNIT_FIELD_NPC_FLAGS) & UNIT_NPC_FLAG_QUESTGIVER)
...
int SendQuestTemplate(Eluna *E, Player *player) {
  ...
  player->PlayerTalkClass->SendQuestGiverQuestDetails(quest, player->GET_GUID(),
                                                      activateAccept);
  return 0;
}
...
int GroupInvite(Eluna *E, Player *player) {
  ...
  if (success) {
    // WorldPackets::Party::PartyInvite partyInvite; // FIXME: Skyfire MoP
    // equivalent missing partyInvite.Initialize(player, 0, true);
    // invited->SendDirectMessage(partyInvite.Write());
    WorldPacket data(SMSG_GROUP_INVITE, 8 + 4 + 1 + 4 + 4 + 10);
    // Simple manual packet for now if possible, or just comment out
    // invited->GetSession()->SendPacket(&data);
    // Commenting out for now to unblock build
    (void)data;
  }
 ...
 *
 * @param uint32 entryId : the ID of the [Creature] or [GameObject] to award
 * credit for.
 */
int KillGOCredit(Eluna *E, Player *player) {
  uint32 entry = E->CHECKVAL<uint32>(2);
  ObjectGuid guid = E->CHECKVAL<ObjectGuid>(3, 0);
...
int RunCommand(Eluna *E, Player *player) {
  ...
  if (std::string(command).length() > 0)
    ChatHandler(player->GetSession()).ParseCommands(command);
  return 0;
}
...
ElunaRegister<Player> PlayerMethods[] = {
    ...
    {"IsImmuneToEnvironmentalDamage",
     &LuaPlayer::IsImmuneToEnvironmentalDamage},
