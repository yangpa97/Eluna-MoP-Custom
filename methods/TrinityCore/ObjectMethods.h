 ...
 * Subclasses of Object, like [WorldObject], extend the block with more data
 * specific to that subclass. Further subclasses, like [Player], extend it even
 * further.
 *
 * A detailed map of all the fields in this data block can be found in the
 * UpdateFields.h file of your emulator (it varies depending on the expansion
 * supported).
 *
 * The GetValue methods in this class (e.g. [Object:GetInt32Value]) provide
 * low-level access to the data block. Other methods, like [Object:HasFlag] and
 * [Object:GetScale], merely wrap the GetValue methods and provide a simpler
 * interface.
 *
...
/**
 * Returns `true` if the [Object] has been added to its [Map], otherwise
 * `false`.
 *
 * @return bool inWorld
 */
...
/**
 * Returns the data at the specified index, casted to a single-precision
...
/**
 * Returns the data at the specified index and offset, casted to an unsigned
 * 8-bit integer.
 *
 * E.g. if you want the second byte at index 10, you would pass in 1 as the
 * offset.
 *
 * @param uint16 index
...
/**
 * Returns the data at the specified index and offset, casted to a signed 16-bit
 * integer.
 *
 * E.g. if you want the second half-word at index 10, you would pass in 1 as the
 * offset.
 *
 * @param uint16 index
 ...
 *
 * This affects the size of a [WorldObject] in-game, but [Item]s don't have a
 * "scale".
 *
 * @return float scale
 */
 ...
 *
 * However on MaNGOS and cMangos creatures and gameobjects inside different maps
 * can share the same GUID but not on the same map.
 *
...
int GetGUIDLow(Eluna *E, Object *obj) {
  E->Push((uint32)(uint64)obj->GetGUID());
  return 1;
}
...
int GetTypeId(Eluna *E, Object *obj) {
  E->Push((uint8)obj->GetTypeId());
  return 1;
}

/**
 * Returns the data at the specified index, casted to an unsigned 64-bit
 * integer.
 *
 * @param uint16 index
 * @return uint64 value
...
/**
 * Sets the data at the specified index to the given value, converted to a
...
/**
 * Sets the data at the specified index to the given value, converted to an
...
/**
 * Sets the data at the specified index and offset to the given value, converted
