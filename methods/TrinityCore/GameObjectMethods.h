 ...
 * @values [GO_STATE_ACTIVE, 0, "show in world as used and not reset (closed
 * door open)"]
 * @values [GO_STATE_READY, 1, "show in world as ready (closed door close)"]
 * @values [GO_STATE_ACTIVE_ALTERNATIVE, 2, "show in world as used in alt way
 * and not reset (closed door open by cannon fire)"]
 *
 * @return [GOState] goState
 */
int GetGoState(Eluna *E, GameObject *go) {
  E->Push((uint32)go->GetGoState());
  return 1;
}
 ...
 * @values [GO_READY, 1, "can be ready but despawned, and then not possible
 * activate until spawn"]
...
int SetLootState(Eluna *E, GameObject *go) {
  ...
  if (state == 0)
    go->SetLootState(LootState::GO_NOT_READY);
  else if (state == 1)
 ...
 *
 * @param bool deleteFromDB : if true, it will delete the [GameObject] from the
 * database
 */
 ...
 *
 * @param uint32 delay = 0 : cooldown time in seconds to restore the
 * [GameObject] back to normal. 0 for infinite duration
 */
 ...
 *
 * @param int32 delay = 0 : cooldown time in seconds to respawn or despawn the
 * object. 0 means never
 */
 ...
 *
 * @param bool spawn = true : if false, it will prevent the [GameObject] from
 * respawning by default
 */
