...
#ifndef ELUNA_TRINITY_UNITMETHODS_H
#define ELUNA_TRINITY_UNITMETHODS_H

#include "Server/WorldPacket.h"
#include "TrinityCore/WorldObjectMethods.h"

// Helper for Trinity's CastSpellExtraArgs in Skyfire
struct CastSpellExtraArgs {
  TriggerCastFlags TriggerFlags = TRIGGERED_NONE;
  Item *CastItem = nullptr;
  ObjectGuid OriginalCaster = ObjectGuid();
  int32 *bp[3] = {nullptr, nullptr, nullptr};
  int32 bp_vals[3] = {0, 0, 0};

  void AddSpellMod(SpellValueMod mod, int32 value) {
    if (mod == SPELLVALUE_BASE_POINT0) {
      bp_vals[0] = value;
      bp[0] = &bp_vals[0];
    } else if (mod == SPELLVALUE_BASE_POINT1) {
      bp_vals[1] = value;
      bp[1] = &bp_vals[1];
    } else if (mod == SPELLVALUE_BASE_POINT2) {
      bp_vals[2] = value;
      bp[2] = &bp_vals[2];
    }
  }
  void SetCastItem(Item *item) { CastItem = item; }
  void SetOriginalCaster(ObjectGuid caster) { OriginalCaster = caster; }
};
 ...
 * @values [MECHANIC_IMMUNE_SHIELD, 29, "Divine (Blessing) Shield/Protection and
 * Ice Block"]
 * @values [MECHANIC_SAPPED, 30, ""]
 * @values [MECHANIC_ENRAGED, 31, ""]
 *
 * @param int32 immunity : new value for the immunity mask
 * @param bool apply = true : if true, the immunity is applied, otherwise it is
 * removed
 */
...
/**
 * Returns true if the [Unit] is in an accessible place for the given
 * [Creature].
 *
...
/**
 * Returns true if the [Unit] provides services like vendor, training and
 * auction.
 *
 * @return bool isTabardDesigner
 */
 ...
 *
 * @param int type = -1 : a valid power type from [Powers] or -1 for the
 * [Unit]'s current power type
...
int GetPowerPct(Eluna *E, Unit *unit) {
  ...
  float percent =
      ((float)unit->GetPower(power) / (float)unit->GetMaxPower(power)) * 100.0f;
 ...
 * @values [LOCALE_ruRU, 8]
 *
 * @param [LocaleConstant] locale = DEFAULT_LOCALE : locale to return the race
 * name in
...
/**
 * Returns a table containing friendly [Unit]'s within given range of the
 * [Unit].
 *
...
int GetFriendlyUnitsInRange(Eluna *E, Unit *unit) {
  ...
  std::list<Unit *> list;
  Skyfire::AnyUnitInObjectRangeCheck checker(unit, range);
  Skyfire::UnitListSearcher<Skyfire::AnyUnitInObjectRangeCheck> searcher(
      unit, list, checker);
  LuaWorldObject::VisitWorldObjects(unit, searcher, range);

  ElunaUtil::ObjectGUIDCheck guidCheck(unit->GET_GUID());
  list.remove_if(guidCheck);

  // Filter for friendly units
  list.remove_if([unit](Unit *u) { return !unit->IsFriendlyTo(u); });

  lua_createtable(E->L, list.size(), 0);
  int tbl = lua_gettop(E->L);
  uint32 i = 0;

  for (std::list<Unit *>::const_iterator it = list.begin(); it != list.end();
       ++it) {
    E->Push(*it);
...
/**
 * Returns a table containing unfriendly [Unit]'s within given range of the
 * [Unit].
 *
 * @param float range = 533.333 : search radius
...
int SetSpeed(Eluna *E, Unit *unit) {
  ...
  (void)forced; // ensure that the variable is referenced in order to pass
                // compiler checks
  if (type >= MAX_MOVE_TYPE)
    return luaL_argerror(E->L, 2, "valid UnitMoveType expected");
...
int SetFFA(Eluna *E, Unit *unit) {
  ...
  if (apply) {
    ...
    for (Unit::ControlList::iterator itr = unit->m_Controlled.begin();
         itr != unit->m_Controlled.end(); ++itr)
      (*itr)->SetByteValue(UNIT_FIELD_SHAPESHIFT_FORM, 1,
                           UNIT_BYTE2_FLAG_FFA_PVP);
  } else {
    unit->RemoveByteFlag(UNIT_FIELD_SHAPESHIFT_FORM, 1,
                         UNIT_BYTE2_FLAG_FFA_PVP);
    for (Unit::ControlList::iterator itr = unit->m_Controlled.begin();
         itr != unit->m_Controlled.end(); ++itr)
      (*itr)->RemoveByteFlag(UNIT_FIELD_SHAPESHIFT_FORM, 1,
                             UNIT_BYTE2_FLAG_FFA_PVP);
  }
...
int EmoteState(Eluna *E, Unit *unit) {
  ...
  unit->SetUInt32Value(UNIT_FIELD_NPC_EMOTESTATE, emoteId);
  return 0;
}
...
int SendChatMessageToPlayer(Eluna *E, Unit *unit) {
  ...
  if (type >= uint32(ChatMsg::MSG_NULL_ACTION))
  ...
  WorldPacket data;
  ChatHandler::BuildChatPacket(data, ChatMsg(type), Language(lang), unit,
                               target, msg);
...
int SendUnitWhisper(Eluna *E, Unit *unit) {
  ...
  uint32 lang = E->CHECKVAL<uint32>(3);
  (void)lang; // ensure that the variable is referenced in order to pass
              // compiler checks
...
int SendUnitSay(Eluna *E, Unit *unit) {
  ...
  if (std::string(msg).length() > 0)
    unit->MonsterSay(msg, (Language)language, unit);
...
int SendUnitYell(Eluna *E, Unit *unit) {
  ...
  if (std::string(msg).length() > 0)
    unit->MonsterYell(msg, (Language)language, unit);
 ...
 * @param int32 bp0 = nil : custom basepoints for [Spell] effect 1. If nil, no
 * change is made
 * @param int32 bp1 = nil : custom basepoints for [Spell] effect 2. If nil, no
 * change is made
 * @param int32 bp2 = nil : custom basepoints for [Spell] effect 3. If nil, no
 * change is made
...
int CastCustomSpell(Eluna *E, Unit *unit) {
  ...
  unit->CastCustomSpell(target, spell, args.bp[0], args.bp[1], args.bp[2],
                        triggered, castItem, nullptr, (uint64)originalCaster);
  return 0;
}

/**
 * Makes the [Unit] cast the spell to the given coordinates, used for area
 * effect spells.
 *
 * @param float x
 * @param float y
 ...
 * @param bool durabilityloss = true : if false, the damage does not do
 * durability damage
 * @param [SpellSchools] school = MAX_SPELL_SCHOOL : school the damage is done
 * in or MAX_SPELL_SCHOOL for direct damage
...
int DealDamage(Eluna *E, Unit *unit) {
  ...
  if (school == MAX_SPELL_SCHOOL) {
    unit->DealDamage(target, damage, NULL, DIRECT_DAMAGE,
                     SPELL_SCHOOL_MASK_NORMAL, NULL, durabilityloss);
    unit->SendAttackStateUpdate(uint32(HitInfo::HITINFO_AFFECTS_VICTIM), target,
                                1, SPELL_SCHOOL_MASK_NORMAL, damage, 0, 0,
                                VictimState::VICTIMSTATE_WOUND, 0);
    return 0;
  }
  ...
  if (Unit::IsDamageReducedByArmor(schoolmask))
    damage = unit->CalcArmorReducedDamage(target, damage, NULL,
  ...
  if (!spell) {
    uint32 absorb = 0;
    uint32 resist = 0;
    unit->CalcAbsorbResist(target, schoolmask, DIRECT_DAMAGE, damage, &absorb,
                           &resist, NULL);

    unit->DealDamageMods(target, damage, &absorb);

    unit->DealDamage(target, damage, NULL, DIRECT_DAMAGE, schoolmask, NULL,
                     false);
    unit->SendAttackStateUpdate(uint32(HitInfo::HITINFO_AFFECTS_VICTIM), target,
                                0, schoolmask, damage, absorb, resist,
                                VictimState::VICTIMSTATE_WOUND, 0);
    return 0;
  }
 ...
 * @param bool durLoss = true : when true, the target's items suffer durability
 * loss
 */
 ...
 * @param bool apply = true : True applies a positive modifier, false applies a
 * negative
 */
int AddFlatStatModifier(Eluna *E, Unit *unit) {
  ...
  unit->HandleStatModifier(UnitMods(UNIT_MOD_STAT_START + statType),
                           (UnitModifierType)modType, value, apply);
  return 0;
}
...
int AddPctStatModifier(Eluna *E, Unit *unit) {
  ...
  unit->HandleStatModifier(
      UnitMods(UNIT_MOD_STAT_START + statType),
      (UnitModifierType)(modType == 0 ? BASE_PCT : TOTAL_PCT), value, true);
  return 0;
}
...
#endif // ELUNA_TRINITY_UNITMETHODS_H
