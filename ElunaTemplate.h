...
#if defined TRACKABLE_PTR_NAMESPACE
template <typename T> struct ElunaConstrainedObjectRef {
...
template <typename T> ElunaConstrainedObjectRef<T> GetWeakPtrFor(T const *obj) {
  ElunaConstrainedObjectRef<Object> ref = GetWeakPtrForObjectImpl(obj);
  return {TRACKABLE_PTR_NAMESPACE static_pointer_cast<T>(ref.Obj),
          ref.BoundMap};
}

#endif

template <typename T> class ElunaObjectImpl : public ElunaObject {
public:
#if defined TRACKABLE_PTR_NAMESPACE
  ElunaObjectImpl(Eluna *E, T const *obj, char const *tname)
      : ElunaObject(E, tname), _obj(GetWeakPtrFor(obj)) {}

  void *GetObjIfValid() const override {
    if (TRACKABLE_PTR_NAMESPACE unique_strong_ref_ptr<T> obj = _obj.Obj.lock())
      if (!E->GetBoundMap() || !_obj.BoundMap ||
          E->GetBoundMap() == _obj.BoundMap)
        return obj.get();

    return nullptr;
  }
#else
  ElunaObjectImpl(Eluna *E, T *obj, char const *tname)
      : ElunaObject(E, tname), _obj(obj), callstackid(E->GetCallstackId()) {}
...
template <typename T> class ElunaObjectValueImpl : public ElunaObject {
public:
...
template <typename T = void> struct ElunaRegister {
  const char *name;
  typename std::conditional<std::is_same<T, void>::value, int (*)(Eluna *),
                            int (*)(Eluna *, T *)>::type mfunc;
  MethodRegisterState regState;
  MethodFlags flags;

  // constructor for class methods
  ElunaRegister(const char *name, int (*func)(Eluna *, T *),
                MethodRegisterState state = METHOD_REG_ALL,
                uint32 flags = METHOD_FLAG_NONE)
      : name(name), mfunc(func), regState(state),
        flags(static_cast<MethodFlags>(flags)) {}

  // constructor for global methods
  ElunaRegister(const char *name, int (*func)(Eluna *),
                MethodRegisterState state = METHOD_REG_ALL,
                uint32 flags = METHOD_FLAG_NONE)
      : name(name), mfunc(func), regState(state),
        flags(static_cast<MethodFlags>(flags)) {}
...
template <typename T = void> class ElunaTemplate {
public:
  ...
  // that will only be needed on lua side and will not be managed by
  // TC/mangos/<core>
  ...
  static void SetMethods(Eluna *E, ElunaRegister<OtherC> const *methodTable,
                         std::size_t N) {
    ASSERT(E);
    if (!methodTable || N == 0)
      return;
    ...
    constexpr bool isGlobal = std::is_same<OtherC, void>::value;
    ...
    for (std::size_t i = 0; i < N; i++) {
      ...
      // if the method should not be registered, push a closure to error output
      // function
      ...
      // if the method is considered unsafe, and unsafe methods have not been
      // enabled, push a closure to error output function
      if (method.flags & METHOD_FLAG_UNSAFE &&
          !sElunaConfig->UnsafeMethodsEnabled()) {
        lua_pushstring(L, method.name);
      ...
      // if the method is considered deprecated, and deprecated methods have not
      // been enabled, push a closure to error output function
      if (method.flags & METHOD_FLAG_DEPRECATED &&
          !sElunaConfig->DeprecatedMethodsEnabled()) {
        lua_pushstring(L, method.name);
      ...
      // if we're in multistate mode, we need to check whether a method is
      // flagged as a world or a map specific method
      if (method.regState != METHOD_REG_ALL) {
        int32 mapId = E->GetBoundMapId();

        // if the method should not be registered, push a closure to error
        // output function
      ...
      lua_pushcclosure(L, thunk_other<OtherC>, 1);
      lua_rawset(L, -3);
    }
  ...
  template <typename OtherC> static int thunk_other(lua_State *L) {
    ElunaRegister<OtherC> *l = static_cast<ElunaRegister<OtherC> *>(
    ...
    int expected = CallDispatcher<OtherC>::Call(E, l, obj);

    int args = lua_gettop(L) - top;
    if (args < 0 || args > expected) {
      ELUNA_LOG_ERROR("[Eluna]: %s returned unexpected amount of arguments %i "
                      "out of %i. Report to devs",
                      l->name, args, expected);
      ASSERT(false);
    }
  ...
  static int thunk(lua_State *L) { return thunk_other<T>(L); }
  ...
  static int Push(Eluna *E, T const *obj) {
    ...
    // Create new userdata
    ElunaObjectType *elunaObject = static_cast<ElunaObjectType *>(
        lua_newuserdata(L, sizeof(ElunaObjectType)));
    if (!elunaObject) {
  ...
  static T *Check(Eluna *E, int narg, bool error = true) {
    ...
    if (!obj) {
      char buff[256];
      snprintf(buff, 256,
               "%s expected, got pointer to nonexisting (invalidated) object "
               "(%s). Check your code.",
  ...
  static int ArithmeticError(lua_State *L) {
    return luaL_error(L, "attempt to perform arithmetic on a %s value", tname);
  }
  static int CompareError(lua_State *L) {
    return luaL_error(L, "attempt to compare %s", tname);
  }
  ...
  static int Concat(lua_State *L) {
    return luaL_error(L, "attempt to concatenate a %s value", tname);
  }
  static int Length(lua_State *L) {
    return luaL_error(L, "attempt to get length of a %s value", tname);
  }
  static int Equal(lua_State *L) {
    Eluna *E = Eluna::GetEluna(L);
    E->Push(E->CHECKOBJ<T>(1) == E->CHECKOBJ<T>(2));
    return 1;
  }
  static int Less(lua_State *L) { return CompareError(L); }
  static int LessOrEqual(lua_State *L) { return CompareError(L); }
  static int Call(lua_State *L) {
    return luaL_error(L, "attempt to call a %s value", tname);
  }

  static int MethodWrongState(lua_State *L) {
    luaL_error(L,
               "attempt to call method '%s' that does not exist for state: %d",
               lua_tostring(L, lua_upvalueindex(1)),
               lua_tointeger(L, lua_upvalueindex(2)));
    return 0;
  }
  static int MethodUnimpl(lua_State *L) {
    luaL_error(
        L,
        "attempt to call method '%s' that is not implemented for this emulator",
        lua_tostring(L, lua_upvalueindex(1)));
    return 0;
  }
  static int MethodUnsafe(lua_State *L) {
    luaL_error(L,
               "attempt to call method '%s' that is flagged as unsafe! to use "
               "this method, enable unsafe methods in the config file",
               lua_tostring(L, lua_upvalueindex(1)));
    return 0;
  }
  static int MethodDeprecated(lua_State *L) {
    luaL_error(L,
               "attempt to call method '%s' that is flagged as deprecated! "
               "this method will be removed in the future. to use this method, "
               "enable deprecated methods in the config file",
               lua_tostring(L, lua_upvalueindex(1)));
    return 0;
  }
};
