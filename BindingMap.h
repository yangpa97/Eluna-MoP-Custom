 ...
 */
template <typename K> class BindingMap : public BaseBindingMap {
private:
  lua_State *L;
  uint64 maxBindingID;
  ...
  struct Binding {
    ...
    Binding(lua_State *L, uint64 id, int functionReference,
            uint32 remainingShots)
        : id(id), L(L), remainingShots(remainingShots),
          functionReference(functionReference) {}

    ~Binding() { luaL_unref(L, LUA_REGISTRYINDEX, functionReference); }
  };
...
public:
  BindingMap(lua_State *L) : L(L), maxBindingID(0) {}

  ~BindingMap() noexcept override = default;

  /*
   * Insert a new binding from `key` to `ref`, which lasts for `shots`-many
   * pushes.
   *
...
template <typename T> struct EventKey {
  ...
  EventKey(T event_id) : event_id(event_id) {}
};
...
template <typename T> struct EntryKey {
  ...
  EntryKey(T event_id, uint32 entry) : event_id(event_id), entry(entry) {}
};
...
template <typename T> struct UniqueObjectKey {
  ...
  UniqueObjectKey(T event_id, ObjectGuid guid, uint32 instance_id)
      : event_id(event_id), guid(guid), instance_id(instance_id) {}
};
...
private:
  ...
  static inline void _hash_combine(result_type &seed, H const &h, T1 const &t1,
                                   T const &...t) {
...
template <> struct hash<ObjectGuid> {
  typedef ObjectGuid argument_type;
  std::size_t operator()(ObjectGuid const &k) const {
    return std::hash<uint64_t>()((uint64_t)const_cast<ObjectGuid &>(k));
  }
};
#endif

template <typename T> struct equal_to<EventKey<T>> {
  bool operator()(EventKey<T> const &lhs, EventKey<T> const &rhs) const {
    return lhs.event_id == rhs.event_id;
  }
};

template <typename T> struct equal_to<EntryKey<T>> {
  bool operator()(EntryKey<T> const &lhs, EntryKey<T> const &rhs) const {
    return lhs.event_id == rhs.event_id && lhs.entry == rhs.entry;
  }
};

template <typename T> struct equal_to<UniqueObjectKey<T>> {
  bool operator()(UniqueObjectKey<T> const &lhs,
                  UniqueObjectKey<T> const &rhs) const {
#if defined ELUNA_SKYFIRE
    return lhs.event_id == rhs.event_id &&
           (uint64) const_cast<ObjectGuid &>(lhs.guid) ==
               (uint64) const_cast<ObjectGuid &>(rhs.guid) &&
           lhs.instance_id == rhs.instance_id;
#else
    return lhs.event_id == rhs.event_id && lhs.guid == rhs.guid &&
           lhs.instance_id == rhs.instance_id;
#endif
  }
};

template <typename T> struct hash<EventKey<T>> {
...
template <typename T> struct hash<EntryKey<T>> {
...
template <typename T> struct hash<UniqueObjectKey<T>> {
