 ...
 * <https://elunaluaengine.github.io/> Copyright (C) 2022 - 2022 Hour of
 * Twilight <https://www.houroftwilight.net/> This program is free software
 * licensed under GPL version 3 Please see the included DOCS/LICENSE.md for more
 * information
 */
...
#if defined ELUNA_TRINITY || ELUNA_MANGOS || defined ELUNA_SKYFIRE
...
void ElunaUpdateListener::handleFileAction(efsw::WatchID /*watchid*/,
...
void ElunaLoader::LoadScripts() {
  // only reload the cache if it is either in a reinit state or not loaded at
  // all
  ...
  std::string lua_folderpath =
      sElunaConfig->GetConfig(CONFIG_ELUNA_SCRIPT_PATH);
  const std::string &lua_path_extra =
      sElunaConfig->GetConfig(CONFIG_ELUNA_REQUIRE_PATH_EXTRA);
  const std::string &lua_cpath_extra =
      sElunaConfig->GetConfig(CONFIG_ELUNA_REQUIRE_CPATH_EXTRA);
  ...
  ELUNA_LOG_INFO("[Eluna]: Searching for scripts in `%s`",
  ...
  // append our custom require paths and cpaths if the config variables are not
  // empty
  ...
  ELUNA_LOG_INFO("[Eluna]: Loaded and precompiled %u scripts in %u ms",
...
int ElunaLoader::LoadBytecodeChunk(lua_State * /*L*/, uint8 *bytes, size_t len,
                                   BytecodeBuffer *buffer) {
  buffer->insert(buffer->end(), bytes, bytes + len);
  return 0;
}

// Finds lua script files from given path (including subdirectories) and pushes
// them to scripts
void ElunaLoader::ReadFiles(lua_State *L, std::string path) {
  ...
  if (fs::exists(someDir) && fs::is_directory(someDir) &&
      !fs::is_empty(someDir)) {
    m_requirePath += path + "/?.lua;" + path + "/?.ext;" + path + "/?.moon;";

    m_requirecPath += path + "/?.dll;" + path + "/?.so;";
    ...
    for (fs::directory_iterator dir_iter(someDir); dir_iter != end_iter;
      ...
      if (fs::is_regular_file(dir_iter->status())) {
        ...
        auto [ptr, ec] = std::from_chars(
            subfolder.data(), subfolder.data() + subfolder.size(), mapId);

        // default to all maps on invalid map id or map id less than -1
        if (ec == std::errc::invalid_argument ||
            ec == std::errc::result_out_of_range || mapId < -1)
          mapId = -1;
...
bool ElunaLoader::CompileScript(lua_State *L, LuaScript &script) {
  ...
  if (script.fileext == ".moon") {
    std::string str =
        "return require('moonscript').loadfile([[" + script.filepath + "]])";
  ...
  if (err != 0) {
    ELUNA_LOG_ERROR(
        "[Eluna]: CompileScript failed to load the Lua script `%s`.",
        script.filename.c_str());
    Eluna::Report(L);
    return false;
  }
  ELUNA_LOG_DEBUG("[Eluna]: CompileScript loaded Lua script `%s`",
                  script.filename.c_str());

  // Everything's OK so far, the script has been loaded, now we need to start
  // dumping it to bytecode.
  err = lua_dump(L, (lua_Writer)LoadBytecodeChunk, &script.bytecode);
  if (err || script.bytecode.empty()) {
    ELUNA_LOG_ERROR("[Eluna]: CompileScript failed to dump the Lua script `%s` "
                    "to bytecode.",
                    script.filename.c_str());
    Eluna::Report(L);
    return false;
  }
  ELUNA_LOG_DEBUG("[Eluna]: CompileScript dumped Lua script `%s` to bytecode.",
                  script.filename.c_str());
...
void ElunaLoader::ProcessScript(lua_State *L, std::string filename,
                                const size_t &filesize,
                                const std::string &fullpath, int32 mapId) {
  ELUNA_LOG_DEBUG("[Eluna]: ProcessScript checking file `%s`",
                  fullpath.c_str());
  ...
  script.modulepath =
      fullpath.substr(0, fullpath.length() - filename.length() - ext.length());
  ...
  ELUNA_LOG_DEBUG("[Eluna]: ProcessScript processed `%s` successfully",
                  fullpath.c_str());
}
...
void ElunaLoader::InitializeFileWatcher() {
  ...
  lua_scriptWatcher =
      lua_fileWatcher.addWatch(lua_folderpath, &elunaUpdateListener, true);
  if (lua_scriptWatcher >= 0) {
...
static bool ScriptPathComparator(const LuaScript &first,
                                 const LuaScript &second) {
...
void ElunaLoader::CombineLists() {
  ...
  std::move(m_extensions.begin(), m_extensions.end(),
            std::back_inserter(m_scriptCache));
  std::move(m_scripts.begin(), m_scripts.end(),
            std::back_inserter(m_scriptCache));
    ...
    sMapMgr.DoForAllMaps([&](Map *map)
#endif
                          {
                            if (mapId == RELOAD_ALL_STATES ||
                                mapId == static_cast<int>(map->GetId()))
