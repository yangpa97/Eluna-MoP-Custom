...
ElunaEventProcessor::~ElunaEventProcessor() { ClearAllEvents(); }
  ...
  while (!eventList.empty() && eventList.begin()->first <= m_time) {
    ...
    if (luaEvent->state == LUAEVENT_STATE_RUN) {
      ...
      if (!obj || (obj && obj->IsInWorld()))
        mgr->E->OnTimedEvent(
            luaEvent->funcRef, delay,
            luaEvent->repeats ? luaEvent->repeats-- : luaEvent->repeats, obj);
...
void ElunaEventProcessor::AddEvent(int funcRef, uint32 min, uint32 max,
                                   uint32 repeats) {
  AddEvent(new LuaEvent(funcRef, min, max, repeats));
}

void ElunaEventProcessor::RemoveEvent(LuaEvent *luaEvent) {
  // Unreference if should and if Eluna was not yet uninitialized and if the lua
  // state still exists
...
void ElunaEventProcessor::QueueDeferredOp(DeferredOpType type, LuaEvent *event,
                                          int eventId, LuaEventState state) {
  DeferredOp op;
  op.type = type;
  op.event = event;
  op.eventId = eventId;
...
void ElunaEventProcessor::ProcessDeferredOps() {
  ...
  static constexpr Handler handlers[] = {
      [](ElunaEventProcessor *self, DeferredOp &op) {
        self->AddEvent(op.event);
      },
      [](ElunaEventProcessor *self, DeferredOp &op) {
        self->SetState(op.eventId, op.state);
      },
      [](ElunaEventProcessor *self, DeferredOp &op) {
        self->SetStates(op.state);
      },
      [](ElunaEventProcessor *self, DeferredOp & /*op*/) {
        self->ClearAllEvents();
      }};
...
void EventMgr::UpdateProcessors(uint32 diff) {
  // iterate a copy because processors may be destroyed during update (creature
  // removed by a script, etc)
