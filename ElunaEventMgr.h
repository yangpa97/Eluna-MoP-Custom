...
enum LuaEventState : uint8 {
  ...
  LUAEVENT_STATE_ABORT, // On next call unregisters reffed function and erases
                        // the data
  LUAEVENT_STATE_ERASE, // On next call just erases the data
};

enum DeferredOpType : uint8 { AddEvent, SetState, SetStates, ClearAll };

enum GlobalEventSpace : uint8 { GLOBAL_EVENTS };

struct LuaEvent {
  LuaEvent(int _funcRef, uint32 _min, uint32 _max, uint32 _repeats)
      : min(_min), max(_max), delay(0), repeats(_repeats), funcRef(_funcRef),
        state(LUAEVENT_STATE_RUN) {}
...
public:
  ...
  ElunaEventProcessor(EventMgr *mgr, WorldObject *obj)
      : m_time(0), obj(obj), mgr(mgr) {}
  ~ElunaEventProcessor();
...
private:
  ...
  void QueueDeferredOp(DeferredOpType type, LuaEvent *event = nullptr,
                       int eventId = 0,
                       LuaEventState state = LUAEVENT_STATE_RUN);
...
private:
  ...
  typedef std::unordered_map<uint64, std::unique_ptr<ElunaEventProcessor>>
      ObjectProcessorMap;
  typedef std::unordered_map<GlobalEventSpace,
                             std::unique_ptr<ElunaEventProcessor>>
      GlobalProcessorsMap;
