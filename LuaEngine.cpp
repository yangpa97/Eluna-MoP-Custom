...
#define NUM_MSG_TYPES NUM_OPCODE_HANDLERS
    ...
    defined ELUNA_SKYFIRE
#define TYPEID_UNIT TypeID::TYPEID_UNIT
#define TYPEID_PLAYER TypeID::TYPEID_PLAYER
#define TYPEID_GAMEOBJECT TypeID::TYPEID_GAMEOBJECT
#define TYPEID_CORPSE TypeID::TYPEID_CORPSE
#endif
...
Eluna::Eluna(Map *map)
    : event_level(0), push_counter(0), boundMap(map), L(NULL) {
  OpenLua();
...
static int PrecompiledLoader(lua_State *L) {
  ...
  auto it = std::find_if(scripts.begin(), scripts.end(),
                         [modname](const LuaScript &script) {
                           return script.filename == modname;
                         });
  if (it == scripts.end()) {
    lua_pushfstring(L, "\n\tno precompiled script '%s' found", modname);
    return 1;
  }
  if (luaL_loadbuffer(L, reinterpret_cast<const char *>(&it->bytecode[0]),
                      it->bytecode.size(), it->filename.c_str())) {
...
void Eluna::OpenLua() {
  ...
  // insert the new loader to the loaders table by shifting other elements down
  // by one
...
void Eluna::CreateBindStores() {
  ...
  CreateBinding<EntryKey<Hooks::GossipEvents>>(
      Hooks::REGTYPE_GAMEOBJECT_GOSSIP);
  ...
  CreateBinding<UniqueObjectKey<Hooks::CreatureEvents>>(
      Hooks::REGTYPE_CREATURE_UNIQUE);
}
...
void Eluna::RunScripts() {
  ...
  ELUNA_LOG_DEBUG("[Eluna]: Running scripts for state: %i, instance: %u",
                  boundMapId, boundInstanceId);
  ...
  for (auto it = scripts.begin(); it != scripts.end(); ++it) {
    // check that the script file is either global or meant to be loaded for
    // this map
    if (it->mapId != -1 && it->mapId != boundMapId) {
      ELUNA_LOG_DEBUG(
          "[Eluna]: `%s` is tagged %i and will not load for map: %i",
          it->filename.c_str(), it->mapId, boundMapId);
      continue;
    }

    // Check that no duplicate names exist
    if (loaded.find(it->filename) != loaded.end()) {
      ELUNA_LOG_ERROR("[Eluna]: Error loading `%s`. File with same name "
                      "already loaded from `%s`, rename either file",
                      it->filepath.c_str(), loaded[it->filename].c_str());
      continue;
    }
    loaded[it->filename] = it->filepath;

    // We call require on the filename to load the script
    // A custom loader is used to load the script from the combined_scripts
    // table The loader is set up in Eluna::OpenLua
    lua_pushvalue(L, -1); // Stack: require, require
    lua_pushstring(L,
                   it->filename.c_str()); // Stack: require, require, filename
    if (ExecuteCall(1, 0)) {
      // Successfully called require on the script
      ELUNA_LOG_DEBUG("[Eluna]: Successfully loaded `%s`",
                      it->filepath.c_str());
      ++count;
      continue;
    }
  ...
  lua_pop(L, 1);
  ELUNA_LOG_INFO(
      "[Eluna]: Executed %u Lua scripts in %u ms for map: %i, instance: %u",
      count, ElunaUtil::GetTimeDiff(oldMSTime), boundMapId, boundInstanceId);
...
bool Eluna::ExecuteCall(int params, int res) {
  ...
  if (!lua_isfunction(L, base)) {
    ELUNA_LOG_ERROR(
        "[Eluna]: Cannot execute call: registered value is %s, not a function.",
        luaL_tolstring(L, base, NULL));
  ...
  // On error print the error and push nils for expected amount of returned
  // values
  if (result) {
    // Stack: errmsg
    Report(L);
...
void Eluna::Push() { lua_pushnil(L); }
void Eluna::Push(const long long l) {
  // pushing pointer to local is fine, a copy of value will be stored, not
  // pointer itself
...
void Eluna::Push(const long l) { Push(static_cast<long long>(l)); }
void Eluna::Push(const unsigned long l) {
  Push(static_cast<unsigned long long>(l));
}
void Eluna::Push(const int i) { lua_pushinteger(L, i); }
void Eluna::Push(const unsigned int u) { lua_pushunsigned(L, u); }
void Eluna::Push(const double d) { lua_pushnumber(L, d); }
void Eluna::Push(const float f) { lua_pushnumber(L, f); }
void Eluna::Push(const bool b) { lua_pushboolean(L, b); }
void Eluna::Push(const std::string &str) { lua_pushstring(L, str.c_str()); }
void Eluna::Push(const char *str) { lua_pushstring(L, str); }
void Eluna::Push(Pet const *pet) { Push<Creature>(pet); }
void Eluna::Push(TempSummon const *summon) { Push<Creature>(summon); }
...
static int CheckIntegerRange(lua_State *luastate, int narg, int min, int max) {
  ...
  if (value < min) {
    snprintf(error_buffer, 64, "value must be greater than or equal to %i",
             min);
    return luaL_argerror(luastate, narg, error_buffer);
  }
...
static unsigned int CheckUnsignedRange(lua_State *luastate, int narg,
                                       unsigned int max) {
  double value = luaL_checknumber(luastate, narg);

  if (value < 0)
    return luaL_argerror(luastate, narg,
  ...
  if (lua_islightuserdata(L, narg)) {
    if (error)
      luaL_argerror(L, narg,
                    "bad argument : userdata expected, got lightuserdata");
    return NULL;
  }

  ElunaObject *elunaObject =
      static_cast<ElunaObject *>(lua_touserdata(L, narg));

  if (!elunaObject || (tname && elunaObject->GetTypeName() != tname)) {
    if (error) {
      char buff[256];
      snprintf(buff, 256, "bad argument : %s expected, got %s",
               tname ? tname : "ElunaObject",
               elunaObject ? elunaObject->GetTypeName()
...
template <typename K> static int cancelBinding(lua_State *L) {
  Eluna *E = Eluna::GetEluna(L);

  uint64 bindingID = E->CHECKVAL<uint64>(lua_upvalueindex(1));

  BindingMap<K> *bindings =
      (BindingMap<K> *)lua_touserdata(L, lua_upvalueindex(2));
...
template <typename K>
static void createCancelCallback(Eluna *e, uint64 bindingID,
                                 BindingMap<K> *bindings) {
  e->Push(bindingID);
...
template <typename K>
int RegisterEntryBinding(Eluna *e,
                         std::underlying_type_t<Hooks::RegisterTypes> regtype,
...
template <typename K>
int RegisterUniqueBinding(Eluna *e,
                          std::underlying_type_t<Hooks::RegisterTypes> regtype,
...
// Saves the function reference ID given to the register type's store for given
// entry under the given event
int Eluna::Register(std::underlying_type_t<Hooks::RegisterTypes> regtype,
                    uint32 entry, ObjectGuid guid, uint32 instanceId,
                    uint32 event_id, int functionRef, uint32 shots) {
  switch (regtype) {
  ...
  case Hooks::REGTYPE_PLAYER:
    if (event_id < Hooks::PLAYER_EVENT_COUNT)
      return RegisterBasicBinding<Hooks::PlayerEvents>(this, regtype, event_id,
                                                       functionRef, shots);
    break;

  case Hooks::REGTYPE_GUILD:
    if (event_id < Hooks::GUILD_EVENT_COUNT)
      return RegisterBasicBinding<Hooks::GuildEvents>(this, regtype, event_id,
                                                      functionRef, shots);
    break;

  case Hooks::REGTYPE_GROUP:
    if (event_id < Hooks::GROUP_EVENT_COUNT)
      return RegisterBasicBinding<Hooks::GroupEvents>(this, regtype, event_id,
                                                      functionRef, shots);
    break;

  case Hooks::REGTYPE_VEHICLE:
    if (event_id < Hooks::VEHICLE_EVENT_COUNT)
      return RegisterBasicBinding<Hooks::VehicleEvents>(this, regtype, event_id,
                                                        functionRef, shots);
    break;

  case Hooks::REGTYPE_BG:
    if (event_id < Hooks::BG_EVENT_COUNT)
      return RegisterBasicBinding<Hooks::BGEvents>(this, regtype, event_id,
                                                   functionRef, shots);
    break;
  ...
  case Hooks::REGTYPE_CREATURE:
    if (event_id < Hooks::CREATURE_EVENT_COUNT) {
      ...
      return RegisterEntryBinding<Hooks::CreatureEvents>(
          this, regtype, entry, event_id, functionRef, shots);
    }
    break;
  ...
  case Hooks::REGTYPE_CREATURE_UNIQUE:
    if (event_id < Hooks::CREATURE_EVENT_COUNT) {
      ...
      return RegisterUniqueBinding<Hooks::CreatureEvents>(
          this, regtype, guid, instanceId, event_id, functionRef, shots);
    }
    break;
  ...
  case Hooks::REGTYPE_CREATURE_GOSSIP:
    if (event_id < Hooks::GOSSIP_EVENT_COUNT) {
      ...
      return RegisterEntryBinding<Hooks::GossipEvents>(
          this, regtype, entry, event_id, functionRef, shots);
    }
    break;
  ...
  case Hooks::REGTYPE_GAMEOBJECT:
    if (event_id < Hooks::GAMEOBJECT_EVENT_COUNT) {
      ...
      return RegisterEntryBinding<Hooks::GameObjectEvents>(
          this, regtype, entry, event_id, functionRef, shots);
    }
    break;
  ...
  case Hooks::REGTYPE_SPELL:
    if (event_id < Hooks::SPELL_EVENT_COUNT)
      return RegisterEntryBinding<Hooks::SpellEvents>(
          this, regtype, entry, event_id, functionRef, shots);
    break;
  ...
  case Hooks::REGTYPE_ITEM:
    if (event_id < Hooks::ITEM_EVENT_COUNT) {
      ...
      return RegisterEntryBinding<Hooks::ItemEvents>(
          this, regtype, entry, event_id, functionRef, shots);
    }
    break;
  ...
  case Hooks::REGTYPE_INSTANCE:
    if (event_id < Hooks::INSTANCE_EVENT_COUNT)
      return RegisterEntryBinding<Hooks::InstanceEvents>(
          this, regtype, entry, event_id, functionRef, shots);
    break;
  }
  luaL_unref(L, LUA_REGISTRYINDEX, functionRef);
  std::ostringstream oss;
  oss << "regtype " << static_cast<unsigned int>(regtype) << ", event "
      << event_id << ", entry " << entry << ", guid " <<
#if defined ELUNA_TRINITY
      guid.ToHexString()
#else
      (uint64)guid
...
void Eluna::CleanUpStack(int number_of_arguments) {
  ...
  lua_pop(L, number_of_arguments + 1); // Add 1 because the caller doesn't know
                                       // about `event_id`. Stack: (empty)
...
/*
 * Call a single event handler that was put on the stack with `Setup` and
 * removes it from the stack.
 *
 * The caller is responsible for keeping track of how many times this should be
 * called.
 */
int Eluna::CallOneFunction(int number_of_functions, int number_of_arguments,
                           int number_of_results) {
  ++number_of_arguments; // Caller doesn't know about `event_id`.
  ASSERT(number_of_functions > 0 && number_of_arguments > 0 &&
         number_of_results >= 0);
  ...
  for (int argument_index = first_argument_index;
       argument_index <= arguments_top; ++argument_index) {
  ...
  return functions_top +
         1; // Return the location of the first result (if any exist).
}
...
CreatureAI *Eluna::GetAI(Creature *creature) {
  for (int i = 1; i < Hooks::CREATURE_EVENT_COUNT; ++i) {
    ...
    auto uniqueKey =
        UKey(event_id, creature->GET_GUID(), creature->GetInstanceId());
  ...
  // Also check for gossip bindings - creatures with only gossip events
  // registered also need ElunaCreatureAI for sGossipHello/sGossipSelect
  for (int i = 0; i < Hooks::GOSSIP_EVENT_COUNT; ++i) {
    Hooks::GossipEvents event_id = (Hooks::GossipEvents)i;
    typedef EntryKey<Hooks::GossipEvents> GKey;
    auto gossipKey = GKey(event_id, creature->GetEntry());
    auto GossipBindings = GetBinding<GKey>(Hooks::REGTYPE_CREATURE_GOSSIP);
    if (GossipBindings->HasBindingsFor(gossipKey))
      return new ElunaCreatureAI(creature);
  }
...
bool Eluna::HasInstanceData(Map const *map) {
  if (!map->Instanceable())
    ...
    return instanceDataRefs.find(map->GetInstanceId()) !=
           instanceDataRefs.end();
}
...
void Eluna::CreateInstanceData(Map const *map) {
  ...
  if (!map->Instanceable()) {
    ...
    // If there's another table that was already stored for the instance, unref
    // it.
...
void Eluna::PushInstanceData(ElunaInstanceAI *ai, bool incrementCounter) {
  ...
  if (!ai->instance->Instanceable())
    ...
    lua_rawgeti(L, LUA_REGISTRYINDEX,
                instanceDataRefs[ai->instance->GetInstanceId()]);
