...
#define START_HOOK(EVENT)                                                      \
  if (!HasBindings(REGTYPE_SERVER)) return;                                    \
...
#define START_HOOK_WITH_RETVAL(EVENT, RETVAL)                                  \
  if (!HasBindings(REGTYPE_SERVER)) return RETVAL;                             \
  auto binding = GetBinding<EventKey<ServerEvents>>(REGTYPE_SERVER);           \
  auto key = EventKey<ServerEvents>(EVENT);                                    \
  if (!binding->HasBindingsFor(key))                                           \
    return RETVAL;
...
void Eluna::OnTimedEvent(int funcRef, uint32 delay, uint32 calls,
                         WorldObject *obj) {
  ASSERT(!event_level);
...
// Weather
void Eluna::OnChange(Weather * /*weather*/, uint32 zone, WeatherState state,
                     float grade) {
...
void Eluna::OnAdd(AuctionHouseObject * /*ah*/, AuctionEntry *entry) {
  ...
  uint32 expiretime = entry->expire_time;
#else
  Player *owner = eObjectAccessor()
      FindPlayer(MAKE_NEW_GUID(entry->owner, 0, HIGHGUID_PLAYER));
  Item *item = eAuctionMgr->GetAItem(entry->itemGUIDLow);
  uint32 expiretime = entry->expire_time;
#endif
