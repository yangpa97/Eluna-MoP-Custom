...
template <typename T, size_t N> constexpr size_t CountOf(const T (&)[N]) {
  return N;
}
...
#define X(ID, VALUE, NAME) {(uint8)ID, NAME},
    PACKET_EVENTS_LIST(X)
#undef X
};
...
#define CREATURE_EVENTS_LIST(X)                                                \
  ...
  X(CREATURE_EVENT_ON_SUMMONED_CREATURE_DESPAWN, 20,                           \
    "on_summoned_creature_despawn")                                            \
...
// SPELL EVENTS
#define SPELL_EVENTS_LIST(X) X(SPELL_EVENT_ON_CAST, 1, "on_cast")
...
#define INSTANCE_EVENTS_LIST(X)                                                \
  ...
  X(INSTANCE_EVENT_ON_CHECK_ENCOUNTER_IN_PROGRESS, 7,                          \
    "on_check_encounter_in_progress")
