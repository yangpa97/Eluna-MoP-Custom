...
private:
  ...
  void CombineLists();
  void ProcessScript(lua_State *L, std::string filename, const size_t &filesize,
                     const std::string &fullpath, int32 mapId);
  bool CompileScript(lua_State *L, LuaScript &script);
  static int LoadBytecodeChunk(lua_State *L, uint8 *bytes, size_t len,
...
public:
  ...
  void handleFileAction(efsw::WatchID /*watchid*/, std::string const &dir,
                        ...
                        std::string oldFilename = "") final override;
};
